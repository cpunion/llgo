...
func TestGenerationConfigMatchesOutcomePrimaryBySourceName(t *testing.T) {
	cfg := generationConfig{functions: []string{"convert"}, checkGlobals: "none"}
	if err := cfg.compile(); err != nil {
		t.Fatal(err)
	}
	if !cfg.matchesFunction("main.convert$outcome", "example.com/p") {
		t.Fatal("source function filter did not select its outcome-primary entry")
	}
	if cfg.matchesFunction("main.convert$coro", "example.com/p") ||
		cfg.matchesFunction("main.convert$coro.resume", "example.com/p") {
		t.Fatal("source function filter selected an auxiliary coroutine entry")
	}
}
