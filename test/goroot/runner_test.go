...
func TestGoRootRunCases(t *testing.T) {
	...
	fmt.Fprintf(os.Stderr, "goroot=%s goversion=%s goos=%s goarch=%s wasm_profile=%s shard=%d/%d cases=%d directive_mode=%s\n", goroot, envInfo.GOVERSION, envInfo.GOOS, envInfo.GOARCH, *flagWasmProfile, *flagShardI, *flagShardN, len(cases), mode.Name)
