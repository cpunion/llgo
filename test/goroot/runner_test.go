...
func TestGoRootRunCases(t *testing.T) {
	...
	for i, tc := range cases {
		...
		t.Run(tc.RelPath, func(t *testing.T) {
			...
			if match, reason := xfails.MatchHostSkip(envInfo.GOVERSION, hostPlatform, tc); match && !wasmTarget {
			...
			if timeout, reason, ok := xfails.MatchTimeoutProfile(envInfo.GOVERSION, targetPlatform, *flagWasmProfile, tc); ok {
				runTimeout = timeout
			...
			match, reason := xfails.MatchProfile(envInfo.GOVERSION, targetPlatform, *flagWasmProfile, tc)
			wasmXFail := false
			if wasmTarget {
				if ok, wasmReason := xfails.MatchWasmProfile(envInfo.GOVERSION, targetPlatform, *flagWasmProfile, tc); ok {
					match, reason, wasmXFail = true, wasmReason, true
				}
			}
			flaky, flakyReason := xfails.MatchFlakyProfile(envInfo.GOVERSION, targetPlatform, *flagWasmProfile, tc)
			notApply, notApplyReason := notApplicable.MatchProfile(envInfo.GOVERSION, targetPlatform, *flagWasmProfile, tc)
