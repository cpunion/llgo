...
func TestGOROOTWasiThreadRunCommand(t *testing.T) {
	...
	want := []string{"--max-threads=128", "--stack-size=1048576", "--heap-size=0", "--dir=.", "--dir=/tmp", "out.wasm", "arg"}
	...
	want = []string{"--max-threads=128", "--stack-size=1048576", "--heap-size=0", "--dir=.", "--dir=/tmp", "go.wasm"}
	if err != nil || app != "iwasm" || !reflect.DeepEqual(args, want) {
