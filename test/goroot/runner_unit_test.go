...
func TestGoTypesDiagnosticAliasesAreNarrow(t *testing.T) {
	aliases := goTypesDiagnosticAliases("unknown field 'foo' in struct literal")
	if len(aliases) != 1 || aliases[0] != "unknown field foo in struct literal" {
		t.Fatalf("unknown-field aliases = %q", aliases)
	}
	for _, message := range []string{
		...
		if aliases := goTypesDiagnosticAliases(message); len(aliases) != 0 {
			t.Fatalf("diagnostic %q acquired broad aliases %q", message, aliases)
		}
	}
}
