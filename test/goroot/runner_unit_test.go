...
func TestRepositoryExpectationsAreSeparated(t *testing.T) {
	...
	for _, entry := range notApplicable.Entries {
		if entry.Version != "" {
			t.Fatalf("not-applicable entry %q must not select a Go version, got %q", entry.Case, entry.Version)
		}
		if entry.Platform != "" && entry.Platform != "js/wasm" && entry.Platform != "wasip1/wasm" {
			t.Fatalf("not-applicable entry %q has unsupported platform scope %q", entry.Case, entry.Platform)
		}
		if !strings.HasPrefix(entry.Reason, "not applicable:") {
