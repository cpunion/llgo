...
void *llgo_symbol(char *name) {
    ...
    /* PE has no RTLD_DEFAULT equivalent. Internal LLGo symbols are resolved
     * from the embedded pclntab; this fallback intentionally covers only
     * symbols exported by the main image. */
...
static const char *llgo_symbol_info(uintptr_t pc, uintptr_t *offset) {
#if defined(_WIN32)
    (void)pc;
    *offset = 0;
    return "";
...
void llgo_stacktrace(int skip, void *ctx, int (*fn)(void *ctx, void *pc, void *offset, void *sp, char *name)) {
    ...
    while (fp) {
        ...
        if (depth >= skip) {
            uintptr_t offset = 0;
            const char *name = llgo_symbol_info(pc, &offset);
