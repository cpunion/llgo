...
func sealSchedulerParkV2(
	...
	for index, caseID := range cases {
		...
		if !idOK || !InitOperation(&operations.records[index], id) ||
			!AttachParkOperation(&g.park, ticket, &operations.records[index], caseID) {
...
func publishSchedulerParkV2(t *testing.T, operations *schedulerParkV2Operations, index int) {
	t.Helper()
...
func detachSchedulerParkV2(t *testing.T, g *G, operations *schedulerParkV2Operations, index int) {
	...
	if !DetachParkOperation(&g.park, operations.ticket, &operations.records[index], operations.ids[index]) {
...
func commitSchedulerParkV2(
	...
	if !PrepareParkSet(task.g, task.handle, task.frame.header, operations.ticket) {
...
func TestSchedulerTaskOnlyCancelDecisionAllowsCleanupPark(t *testing.T) {
	...
	if !RequestParkCancel(&task.g.park, cleanupPark.ticket, ParkCancelOperation) {
...
func TestSchedulerParkSetAndLegacyWaitPreserveQueueOrder(t *testing.T) {
	...
	if p.waitHead != legacy.g || p.waitTail != v2.g || legacy.g.nextWait != v2.g {
		t.Fatal("mixed wait insertion order changed")
	}
...
func TestPrepareParkSetFailsClosedForUnsealedStaleAndDuplicate(t *testing.T) {
	...
	task.frame.header.SuspendReason = uint16(SuspendPark)
	task.frame.header.Lifecycle = uint16(FrameSuspended)
	if PrepareParkSet(task.g, task.handle, task.frame.header, ticket) || task.g.park.phase != parkPreparing || task.g.pending.kind != pendingNone {
	...
	stale := ticket
	stale.generation++
	if PrepareParkSet(task.g, task.handle, task.frame.header, stale) || task.g.park.phase != parkSealed || task.g.pending.kind != pendingNone {
		t.Fatal("stale park ticket partially committed")
	}
	if !PrepareParkSet(task.g, task.handle, task.frame.header, ticket) || task.g.park.phase != parkParked || task.g.pending.kind != pendingParkSet {
	...
	if !RequestParkCancel(&task.g.park, ticket, ParkCancelOperation) {
