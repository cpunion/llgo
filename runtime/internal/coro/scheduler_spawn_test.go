...
func TestSpawnParkCompletionRacesPeerTerminalUsingStableP(t *testing.T) {
	p := new(P)
	parent := newYieldingTestG(t, "park-parent")
	if !Enqueue(p, parent.g) {
		t.Fatal("enqueue park parent")
	}
	if got, ok := NextRunnable(p); !ok || got != parent.g {
		t.Fatal("dequeue park parent")
	}
	parentAction := beginSpawnTestResume(t, p, parent)
	child := new(G)
	if !BeginSpawn(parent.g, child, unsafe.Pointer(child), TaskStorageSize()) {
		t.Fatal("begin parked child")
	}
	handle := unsafe.Pointer(new(byte))
	frame, descriptor := newSpawnTestFrame(t, child, handle, 0, 1)
	if !CommitSpawn(parent.g, child, handle) {
		t.Fatal("commit parked child")
	}
	yieldSpawnTestG(t, p, parent.g, parent.frame, parentAction)
	if got, ok := NextRunnable(p); !ok || got != child {
		t.Fatal("dequeue parked child")
	}
	childAction := beginSpawnTestChildResume(t, p, child, frame)
	token := new(WaitToken)
	ticket, ok := ArmWait(token)
	if !ok {
		t.Fatal("arm child park token")
	}
	frame.header.SuspendReason = uint16(SuspendPark)
	frame.header.Lifecycle = uint16(FrameSuspended)
	if !PreparePark(child, handle, frame.header, token, ticket) {
		t.Fatal("prepare child park")
	}
	if action, ok := Resumed(p, child, childAction); !ok || action.Kind != ActionPark || !HasWaiting(p) {
		t.Fatal("commit child park")
	}

	if got, ok := NextRunnable(p); !ok || got != parent.g {
		t.Fatal("dequeue parent beside parked child")
	}
	parentAction = beginSpawnTestResume(t, p, parent)
	...
	go func() {
		<-start
		producerDone <- CompleteWait(token, ticket) && RequestSchedule(p)
	}()
	close(start)
	completeSpawnTestG(t, p, parent.g, parent.frame, parentAction)
	if !<-producerDone {
		t.Fatal("P-only completion producer was rejected")
	}
	if !DeadG(parent.g) || TerminalG(p, parent.g) {
		t.Fatal("dead peer was mistaken for a P-wide terminal program")
	}
	if count, ok := PollReady(p); !ok || count != 1 || HasWaiting(p) {
		t.Fatalf("promote spawned parked child = (%d, %t), waiting=%t", count, ok, HasWaiting(p))
	}
	if got, ok := NextRunnable(p); !ok || got != child {
		t.Fatal("dequeue completed parked child")
	}
	childAction = beginSpawnTestChildResume(t, p, child, frame)
	completeSpawnTestG(t, p, child, frame, childAction)
	if !ReclaimableG(child) {
		t.Fatal("completed parked child is not reclaimable")
	}
	if _, _, ok := ReleaseTaskStorage(child); !ok {
		t.Fatal("release completed parked child task")
	}
	if !TerminalG(p, parent.g) || !TerminalG(p, child) {
		t.Fatal("park/terminal race retained scheduler state")
	}
	runtime.KeepAlive(parent.frame.memory)
	runtime.KeepAlive(frame.memory)
	runtime.KeepAlive(descriptor)
...
func TestMainReturnWithParkedSpawnFailsClosed(t *testing.T) {
	p := new(P)
	main := newYieldingTestG(t, "main")
	if !Enqueue(p, main.g) {
		t.Fatal("enqueue main")
	}
	if got, ok := NextRunnable(p); !ok || got != main.g {
		t.Fatal("dequeue main")
	}
	mainAction := beginSpawnTestResume(t, p, main)
	child := new(G)
	if !BeginSpawn(main.g, child, unsafe.Pointer(child), TaskStorageSize()) {
		t.Fatal("begin main child")
	}
	handle := unsafe.Pointer(new(byte))
	frame, descriptor := newSpawnTestFrame(t, child, handle, 0, 1)
	if !CommitSpawn(main.g, child, handle) {
		t.Fatal("commit main child")
	}
	yieldSpawnTestG(t, p, main.g, main.frame, mainAction)
	if got, ok := NextRunnable(p); !ok || got != child {
		t.Fatal("dequeue main child")
	}
	childAction := beginSpawnTestChildResume(t, p, child, frame)
	token := new(WaitToken)
	ticket, ok := ArmWait(token)
	if !ok {
		t.Fatal("arm main child wait")
	}
	frame.header.SuspendReason = uint16(SuspendPark)
	frame.header.Lifecycle = uint16(FrameSuspended)
	if !PreparePark(child, handle, frame.header, token, ticket) {
		t.Fatal("prepare main child park")
	}
	if action, ok := Resumed(p, child, childAction); !ok || action.Kind != ActionPark {
		t.Fatal("park main child")
	}
	if got, ok := NextRunnable(p); !ok || got != main.g {
		t.Fatal("dequeue main after child park")
	}
	mainAction = beginSpawnTestResume(t, p, main)
	completeSpawnTestG(t, p, main.g, main.frame, mainAction)
	if !DeadG(main.g) || TerminalG(p, main.g) || !HasWaiting(p) {
		t.Fatal("main return drained or accepted a suspended background G")
	}
	if got, ok := NextRunnable(p); !ok || got != nil || !HasWaiting(p) {
		t.Fatalf("parked background G became runnable after main return: (%p, %t)", got, ok)
	}
	runtime.KeepAlive(main.frame.memory)
	runtime.KeepAlive(frame.memory)
	runtime.KeepAlive(descriptor)
	runtime.KeepAlive(child)
}
