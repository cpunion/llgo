...
func retireCompletedRegistration(t *testing.T, table *WaitRegistrationTable, handle WaitRegistrationHandle) {
	t.Helper()
	if result := table.BeginClose(handle); result != WaitRegistrationCloseStarted {
		t.Fatalf("begin completed registration close = %d", result)
	}
	if result, ok := table.ConfirmQuiesced(handle); !ok || result != WaitCancelCompletionWon {
		t.Fatalf("confirm completed registration = (%d, %t)", result, ok)
	}
	if !table.Retire(handle) {
		t.Fatal("retire completed registration")
	}
}
...
func TestPostWaitAndRequestDefersSchedulerMutation(t *testing.T) {
	table := new(WaitRegistrationTable)
	registry := new(ExecutorRegistry)
	p := new(P)
	token, ticket, wait := registerTestWait(t, table, p)
	executor := registerTestExecutor(t, registry)
	result := PostWaitAndRequest(table, wait, registry, executor)
	if result.Wait != WaitRegistrationPosted || result.Executor != ExecutorRequestPublished {
		t.Fatalf("post wait and request = %+v", result)
	}
	if !table.Pending() || !registry.ObserveRequested(executor) {
		t.Fatal("platform ingress did not publish both durable gates")
	}
	if outcome, ok := WaitOutcomeOf(token, ticket); ok || outcome != WaitOutcomeInvalid {
		t.Fatal("platform ingress mutated scheduler wait token")
	}
	duplicate := PostWaitAndRequest(table, wait, registry, executor)
	if duplicate.Wait != WaitRegistrationPostDuplicate || duplicate.Executor != ExecutorRequestInvalid {
		t.Fatalf("duplicate post wait and request = %+v", duplicate)
	}
	if drained, ok := table.Drain(); !ok || drained != 1 {
		t.Fatalf("drain wait = (%d, %t)", drained, ok)
	}
	if cleared, ok := registry.Acknowledge(executor); !ok || !cleared {
		t.Fatal("acknowledge drained executor")
	}
	...
	if !table.CanRelease() || !registry.CanRelease() {
		t.Fatal("completed ingress retained stable owners")
	}
}

func TestExecutorAcknowledgeRequiresDurableSourceRecheck(t *testing.T) {
	table := new(WaitRegistrationTable)
	registry := new(ExecutorRegistry)
	p := new(P)
	firstToken, firstTicket, firstWait := registerTestWait(t, table, p)
	secondToken, secondTicket, secondWait := registerTestWait(t, table, p)
	executor := registerTestExecutor(t, registry)

	if result := PostWaitAndRequest(table, firstWait, registry, executor); result.Wait != WaitRegistrationPosted || result.Executor != ExecutorRequestPublished {
		t.Fatalf("first ingress = %+v", result)
	}
	if drained, ok := table.Drain(); !ok || drained != 1 {
		t.Fatalf("first drain = (%d, %t)", drained, ok)
	}
	// The second producer arrives after Drain but before Acknowledge. Its
	// executor request coalesces into the bit that Acknowledge is about to
	// clear, while its registration remains the source of truth.
	if result := PostWaitAndRequest(table, secondWait, registry, executor); result.Wait != WaitRegistrationPosted || result.Executor != ExecutorRequestCoalesced {
		t.Fatalf("coalesced second ingress = %+v", result)
	}
	if cleared, ok := registry.Acknowledge(executor); !ok || !cleared {
		t.Fatal("acknowledge coalesced request")
	}
	if !table.Pending() {
		t.Fatal("acknowledge erased durable source")
	}
	if drained, ok := table.Drain(); !ok || drained != 1 {
		t.Fatalf("mandatory post-ack recheck drain = (%d, %t)", drained, ok)
	}
	...
	retireCompletedRegistration(t, table, firstWait)
	retireCompletedRegistration(t, table, secondWait)
	retireTestExecutor(t, registry, executor)
}

func TestExecutorFinalIdleRecheckClosesPostRequestWindow(t *testing.T) {
	table := new(WaitRegistrationTable)
	registry := new(ExecutorRegistry)
	p := new(P)
	token, ticket, wait := registerTestWait(t, table, p)
	...
	// Model a producer paused between publishing its durable wait and
	// requesting the executor. ArmIdle can win, but the required source recheck
	// observes the wait and cancels the sleep before the delayed request.
	if result := table.Post(wait); result != WaitRegistrationPosted {
		t.Fatalf("durable post = %d", result)
	}
	if !registry.ArmIdle(executor) || !table.Pending() {
		t.Fatal("model final idle recheck")
	}
	if left, ok := registry.LeaveIdle(executor); !ok || !left {
		t.Fatal("cancel idle after durable recheck")
	}
	if result := registry.Request(executor); result != ExecutorRequestPublished {
		t.Fatalf("delayed executor request = %d", result)
	}
	if drained, ok := table.Drain(); !ok || drained != 1 {
		t.Fatalf("drain delayed ingress = (%d, %t)", drained, ok)
	}
	if cleared, ok := registry.Acknowledge(executor); !ok || !cleared {
		t.Fatal("acknowledge delayed ingress")
	}
	consumeRegisteredOutcome(t, token, ticket, WaitOutcomeCompleted)
	retireCompletedRegistration(t, table, wait)
	retireTestExecutor(t, registry, executor)
...
func TestExecutorCloseStillDrainsPostBeforeRequest(t *testing.T) {
	table := new(WaitRegistrationTable)
	registry := new(ExecutorRegistry)
	p := new(P)
	token, ticket, wait := registerTestWait(t, table, p)
	...
	// Model a platform shim paused after its durable post. Close can win the
	// executor gate before the delayed Request, so shutdown must not use the
	// advisory request bit as proof that all sources were drained.
	if result := table.Post(wait); result != WaitRegistrationPosted {
		t.Fatalf("post before executor close = %d", result)
	}
	if !registry.BeginClose(executor) {
		t.Fatal("close executor during post/request window")
	}
	if result := registry.Request(executor); result != ExecutorRequestClosed {
		t.Fatalf("delayed request after close = %d", result)
	}
	// This unconditional drain models the required step after the physical
	// backend has joined the complete shim, including its doorbell tail.
	if drained, ok := table.Drain(); !ok || drained != 1 {
		t.Fatalf("final shutdown drain = (%d, %t)", drained, ok)
	}
	consumeRegisteredOutcome(t, token, ticket, WaitOutcomeCompleted)
	if !registry.ConfirmQuiesced(executor) || !registry.Retire(executor) {
		t.Fatal("quiesce executor after final drain")
	}
	retireCompletedRegistration(t, table, wait)
}
