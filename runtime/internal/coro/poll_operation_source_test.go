...
) (*ExecutorSourceSet, *PollOperationSource) {
	t.Helper()
...
func finishPollV2Test(
	...
	if !unbindExecutorSourceSet(sources, p) || !poll.CanRelease() {
...
func TestPollOperationV2SharesWaitSetWithTimerV2(t *testing.T) {
	...
	sources, poll := bindPollV2TestSources(t, p, RouteID(7), timers)
	...
	finishWaitTestTask(t, p, park.task, action)
	if !unbindExecutorSourceSet(sources, p) || !poll.CanRelease() || !timers.CanRelease() {
