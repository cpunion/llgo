...
// target identity before the function becomes a uintptr; the compiler derives
// the carrier width from the exact sink. The operations have no caller-thread
// affinity or managed callback, and pointer arguments remain rooted by the
// suspended managed caller until worker completion.
