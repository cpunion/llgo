...
func init() {
	...
	// Replaces the runtime core's plain signal() handler: hardware faults
	// (nil derefs compiled with null_pointer_is_valid, faults inside C
	// code, x86 integer division) carry the interrupted pc/fp, so the
	// panic snapshot can start at the fault site — the handler's own
	// chain dead-ends at the signal trampoline. Also covers SIGFPE and
	// SIGBUS, which previously killed the process.
...
func onFault(pc, fp uintptr, sig int32) {
	...
	// this one turns into an ordinary (recoverable) panic.
	c_faultCaptureDone()
	rtdebug.PanicSignal(int(sig))
}

// fpWalkFrom walks the frame-pointer chain starting at an arbitrary frame
// pointer (a fault context's fp). Chain-discipline guards only: it runs in
// signal-handler context where the first-use frame tables cannot be built,
// so the program-text bound is applied later, when the snapshot is
// consumed in normal context (trimPlumbingPCs).
func fpWalkFrom(fp uintptr, pc []uintptr) int {
	n := 0
	const maxFrames = 4096
	wordSize := unsafe.Sizeof(uintptr(0))
	for i := 0; fp != 0 && n < len(pc) && i < maxFrames; i++ {
		// The arithmetic guards below cannot prove the slot is mapped —
		// probe before dereferencing (this walk runs in fault context,
		// where a second fault would otherwise recurse).
