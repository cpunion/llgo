...
func makeWasmFunc(ft *abi.FuncType, fn func([]Value) []Value, recoverTo unsafe.Pointer) Value {
	...
	if code == nil {
		...
		// appear at a compiled call site. It is still callable via reflection.
		// FuncOf returns the compiled descriptor when a static type exists.
...
func wasmMakeFuncInvoke(env unsafe.Pointer, args, results *unsafe.Pointer) {
	...
	for i, typ := range fd.ftyp.In {
		// The generated stub's argument slots belong to its stack. Callback
		// Values may escape, so copy them into independent typed allocations.
		ins[i] = New(toType(toRuntimeType(typ))).Elem()
		ins[i].Set(ffiToValue(argv[i], typ))
	}
