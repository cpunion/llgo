...
func forkAndExecInChild1(argv0 *byte, argv, envv []*byte, chroot, dir *byte, attr *syscall.ProcAttr, sys *syscall.SysProcAttr, pipe int) (pid uintptr, pidfd int32, err1 syscall.Errno, mapPipe [2]int, locked bool) {
	...
	locked = true
	if clone3 != nil {
		pid = uintptr(C.sys_clone3((*C.struct_clone_args)(unsafe.Pointer(clone3)), C.size_t(unsafe.Sizeof(*clone3))))
		if pid < 0 {
	...
	if sys.Unshareflags != 0 {
		...
		if sys.Unshareflags&syscall.CLONE_NEWUSER != 0 && sys.GidMappings != nil {
			dirfd = int(_AT_FDCWD)
			fd1 = uintptr(C.sys_openat(C.int(dirfd), (*C.char)(unsafe.Pointer(&psetgroups[0])), C.int(syscall.O_WRONLY), 0))
			if fd1 < 0 {
				err1 = syscall.Errno(C.llgo_errno())
				goto childerror
			}
			pid = uintptr(C.write(C.int(fd1), unsafe.Pointer(&setgroups[0]), C.size_t(len(setgroups))))
			if pid < 0 {
				err1 = syscall.Errno(C.llgo_errno())
				goto childerror
			}
			...
			fd1 = uintptr(C.sys_openat(C.int(dirfd), (*C.char)(unsafe.Pointer(&pgid[0])), C.int(syscall.O_WRONLY), 0))
			if fd1 < 0 {
				err1 = syscall.Errno(C.llgo_errno())
				goto childerror
			}
			pid = uintptr(C.write(C.int(fd1), unsafe.Pointer(&gidmap[0]), C.size_t(len(gidmap))))
			if pid < 0 {
				err1 = syscall.Errno(C.llgo_errno())
				goto childerror
			}
		...
		if sys.Unshareflags&syscall.CLONE_NEWUSER != 0 && sys.UidMappings != nil {
			dirfd = int(_AT_FDCWD)
			fd1 = uintptr(C.sys_openat(C.int(dirfd), (*C.char)(unsafe.Pointer(&puid[0])), C.int(syscall.O_WRONLY), 0))
			if fd1 < 0 {
				err1 = syscall.Errno(C.llgo_errno())
				goto childerror
			}
			pid = uintptr(C.write(C.int(fd1), unsafe.Pointer(&uidmap[0]), C.size_t(len(uidmap))))
			if pid < 0 {
				err1 = syscall.Errno(C.llgo_errno())
				goto childerror
			}
	...
	if pipe < nextfd {
		if C.dup3(C.int(pipe), C.int(nextfd), C.int(syscall.O_CLOEXEC)) != 0 {
			err1 = syscall.Errno(C.llgo_errno())
			goto childerror
		}
		pipe = nextfd
	...
	for i = 0; i < len(fd); i++ {
		if fd[i] >= 0 && fd[i] < i {
			...
			if C.dup3(C.int(fd[i]), C.int(nextfd), C.int(syscall.O_CLOEXEC)) != 0 {
				err1 = syscall.Errno(C.llgo_errno())
				goto childerror
			}
			fd[i] = nextfd
	...
	for i = 0; i < len(fd); i++ {
		...
		if C.dup3(C.int(fd[i]), C.int(i), 0) != 0 {
			err1 = syscall.Errno(C.llgo_errno())
			goto childerror
		}
	}
