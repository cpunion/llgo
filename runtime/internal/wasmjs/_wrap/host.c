...
EM_JS(void, llgo_js_host, (int32_t op, uint64_t *frame), {
    ...
    const text = () => new TextDecoder('utf-8').decode(HEAPU8.subarray(integer(2), integer(2) + integer(3)));
    ...
    switch (op) {
        ...
        case 11: {
            const value = new TextEncoder().encode(String(load(address)));
            store(address, value);
