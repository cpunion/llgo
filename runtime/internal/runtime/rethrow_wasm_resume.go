...
// Rethrow returns panic and Goexit handling to the resumable scheduler. The
// scheduler then redirects the explicit frame chain to the defer owner.
func Rethrow(link *Defer) {
	...
	if unwind := gp.context.platform.unwind; unwind != nil {
		restoreWasmResumeGCRoot(gp.context.platform.unwindRoot)
