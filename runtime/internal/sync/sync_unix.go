...
// pthread_mutex_init initializes caller-owned fixed-size state; it neither
// acquires the mutex nor invokes a callback.
//
//llgo:coro noblock
//go:linkname pthreadMutexInit C.pthread_mutex_init
func pthreadMutexInit(m *Mutex, attr *MutexAttr) c.Int

// Destroy releases caller-owned metadata after its lifecycle is quiescent.
// The ordinary foreign default is sufficient because it does not wait for a
// future unlock, invoke a callback, or retain either argument.
//
...
// host-stack invocation; managed callers retain BlockForeign/WaitForeign and
// Direct raw-host execution is inferred at each compiler-owned invocation.
//
//go:linkname pthreadMutexLock C.pthread_mutex_lock
func pthreadMutexLock(m *Mutex) c.Int

// Unlock releases ownership and never waits for a future owner.
//
//llgo:coro noblock
...
// run or retain arguments. They use the ordinary foreign default rather than a declaration-wide sync capability.
//
...
// Wait releases and reacquires m while waiting for another thread. It is legal
// only in an audited raw host-stack invocation; managed callers retain
// BlockForeign/WaitForeign and direct raw-host execution is inferred at each
