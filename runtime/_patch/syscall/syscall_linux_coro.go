...
// The fixed C leaves are linked by runtime's native LLGoFiles manifest so the
// source-patched standard package does not depend on a patch-directory object.
//
...
func linuxSyscallErrno(r1, errno uintptr) Errno {
	if r1 == ^uintptr(0) {
		return Errno(errno)
	}
	return 0
