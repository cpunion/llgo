...
func unsupportedWebAssemblySyscall() (r1, r2 uintptr, err Errno) {
...
func hostSockaddr(sa Sockaddr) (coroHostSockaddrV1, error) {
	switch address := sa.(type) {
	case *SockaddrInet4:
		if address == nil || address.Port < 0 || address.Port > 1<<16-1 {
			return coroHostSockaddrV1{}, EINVAL
		}
...
func optionalSockaddrFromHost(address *coroHostSockaddrV1) (Sockaddr, error) {
...
func Accept4(fd, flags int) (newfd int, sa Sockaddr, err error) {
	_ = flags
	return Accept(fd)
}
