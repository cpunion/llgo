...
// These replacement declarations are exact capability metadata for the
// upstream zsyscall FuncPCABI0 operands of the same names. Source-patch
// selection replaces only those declarations with worker-safe C targets.
// Process- or thread-affine targets (fork, exec, pthread operations, and
// unknown code words) intentionally have no replacement and remain closed.
//
...
func llgoErrno32(result, errno uintptr) Errno {
	if int32(result) == -1 {
		return Errno(errno)
	}
	return 0
...
func llgoErrnoWord(result, errno uintptr) Errno {
	if result == ^uintptr(0) {
		return Errno(errno)
	}
	return 0
