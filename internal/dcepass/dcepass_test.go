	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := llvm.VerifyModule(dst, llvm.ReturnStatusAction); err != nil {
				t.Fatalf("verify emitted cross-context overrides: %v\n%s", err, dst.String())
			}
...
func TestEmitStrongTypeOverridesReplacesMismatchedDeclaration(t *testing.T) {
	ctx := llvm.NewContext()
	defer ctx.Dispose()
	src := parseModule(t, &ctx, filepath.Join("testdata", "method_slots", "in.ll"))
	defer src.Dispose()
	dst := ctx.NewModule("dst")
	defer dst.Dispose()

	wrongType := ctx.StructType([]llvm.Type{ctx.Int8Type()}, false)
	llvm.AddGlobal(dst, wrongType, taskTypeName)
	EmitStrongTypeOverrides(dst, []llvm.Module{src}, map[string][]int{taskTypeName: {1}}, false)

	override := dst.NamedGlobal(taskTypeName)
	if override.IsNil() || override.Initializer().IsNil() {
		t.Fatal("mismatched declaration was not replaced with a strong override")
	}
	if override.GlobalValueType() != override.Initializer().Type() {
		t.Fatalf("override type = %s, initializer type = %s", override.GlobalValueType(), override.Initializer().Type())
	}
	if err := llvm.VerifyModule(dst, llvm.ReturnStatusAction); err != nil {
		t.Fatalf("verify replacement override: %v\n%s", err, dst.String())
	}
}
...
@target_as1 = external addrspace(1) global i8
...
@unsupported_expr = global ptr addrspacecast (ptr addrspace(1) @target_as1 to ptr)
