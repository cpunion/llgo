...
func (e *overrideEmitter) emitTypeOverride(srcType, methodsVal llvm.Value, elemTy llvm.Type, keepIdx map[int]bool, verbose bool) {
	...
	if dstType.GlobalValueType() != override.Type() {
		...
		// packages. The entry module already contains a declaration selected
		// from the program-wide ABI registry. Use the exact cloned initializer
		// type for the strong definition and redirect the opaque-pointer uses.
		name := dstType.Name()
		replacement := llvm.AddGlobal(e.dst, override.Type(), name+".llgo.dce")
		dstType.ReplaceAllUsesWith(replacement)
		dstType.EraseFromParentAsGlobal()
		replacement.SetName(name)
		dstType = replacement
		e.values[srcType] = dstType
	}
	dstType.SetInitializer(override)
