	...
	for _, root := range p.roots {
		if root.Function == target || identified && root.ID == targetID {
			sources = append(sources, fmt.Sprintf(
				"root(%s managed=%s raw=%t scheduled=%t emission=%t ingress=%t)",
				root.Function, root.ManagedDemand, root.RawPlainDemand,
				root.ScheduledEntry, root.EmissionEntry, root.IngressEntry,
			))
		}
	}
	if identified {
