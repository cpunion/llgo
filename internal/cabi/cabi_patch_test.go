...
func TestClosureContextAttributeRemappedByCABI(t *testing.T) {
	llvm.InitializeAllTargets()
	llvm.InitializeAllTargetMCs()
	llvm.InitializeAllTargetInfos()
...
define %Value @callee(ptr %g, ptr %out, ptr nest %ctx, %Value %value) {
entry:
  ret %Value %value
}

define %Value @caller(ptr %g, ptr %out, ptr nest %ctx, %Value %value) {
entry:
  %result = call %Value @callee(ptr %g, ptr %out, ptr nest %ctx, %Value %value)
  ret %Value %result
}
`
	ctx := llvm.NewContext()
	defer ctx.Dispose()
	path := filepath.Join(t.TempDir(), "closure_context.ll")
	if err := os.WriteFile(path, []byte(testIR), 0o644); err != nil {
		t.Fatal(err)
	}
	buf, err := llvm.NewMemoryBufferFromFile(path)
	...
	prog := llssa.NewProgram(&llssa.Target{GOOS: "linux", GOARCH: "amd64"})
	defer prog.Dispose()
	tr := NewTransformer(prog, "x86_64-unknown-linux-gnu", "", ModeAllFunc, true)
	tr.TransformModule("test", mod)

	nest := llvm.AttributeKindID("nest")
	callee := mod.NamedFunction("callee")
	// %Value is returned through a newly inserted sret parameter, so the old
	// third parameter must move from attribute index 3 to index 4.
	if attr := callee.GetEnumAttributeAtIndex(4, nest); attr.IsNil() {
		t.Fatalf("C ABI lowering lost/remapped nest to the wrong definition parameter:\n%s", callee.String())
	}
	if attr := callee.GetEnumAttributeAtIndex(3, nest); !attr.IsNil() {
		t.Fatalf("C ABI lowering left nest on the old definition parameter:\n%s", callee.String())
	}
	...
	if attr := nestedCall.GetCallSiteEnumAttribute(4, nest); attr.IsNil() {
		t.Fatalf("C ABI lowering lost/remapped nest to the wrong call parameter:\n%s", caller.String())
	}
	if attr := nestedCall.GetCallSiteEnumAttribute(3, nest); !attr.IsNil() {
		t.Fatalf("C ABI lowering left nest on the old call parameter:\n%s", caller.String())
	}
	if err := llvm.VerifyModule(mod, llvm.ReturnStatusAction); err != nil {
		t.Fatalf("C ABI closure-context module is invalid: %v\n%s", err, mod.String())
	}
}
