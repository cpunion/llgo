...
`
	for _, layout := range []string{"e-p:32:32-i64:64-n32:64-S128", "e-p:64:64-i64:64-n32:64-S128"} {
		t.Run(layout, func(t *testing.T) {
			...
			frame = NewGCRootFrame(mod, fn, 1, td.PointerSize(), false)
			PopGCRootFrame(mod, fn, frame)
			if err := llvm.VerifyModule(mod, llvm.ReturnStatusAction); err != nil {
				t.Fatal(err)
			}
...
func hasPointerRootStore(value llvm.Value) bool {
	for use := value.FirstUse(); !use.IsNil(); use = use.NextUse() {
		...
		if !store.IsNil() && store.Operand(0) == value {
			return true
		}
	}
	return false
