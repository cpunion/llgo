...
// GCRootFrame is the common shadow-frame layout used by frontend roots and
// temporaries introduced later by ABI lowering.
...
func NewGCRootFrame(m llvm.Module, fn llvm.Value, count, pointerSize int, wasm bool) GCRootFrame {
	ctx := m.Context()
	b := ctx.NewBuilder()
	defer b.Dispose()
	ptr := llvm.PointerType(ctx.Int8Type(), 0)
	array := llvm.ArrayType(ptr, count)
	typ := ctx.StructType([]llvm.Type{ptr, ptr, array}, false)
	...
	chain := rootGlobal(m, "llvm_gc_root_chain", ptr, pointerSize)
	replay := rootGlobal(m, "llvm_gc_root_sjlj_replaying", ctx.Int1Type(), 1)
	prev := b.CreateLoad(ptr, chain, "")
	prevSlot := b.CreateStructGEP(typ, frame, 0, "")
	reentered := b.CreateICmp(llvm.IntEQ, prev, frame, "")
	...
	b.CreateStore(rootMap(m, fn, count, wasm), b.CreateStructGEP(typ, frame, 1, ""))
	for _, slot := range slots {
		b.CreateStore(llvm.ConstNull(ptr), slot)
	}
	b.CreateStore(frame, chain)
	b.CreateBr(entry)
	return GCRootFrame{Frame: frame, Prev: prevSlot, Slots: slots}
}
...
func PopGCRootFrame(m llvm.Module, fn llvm.Value, frame GCRootFrame) {
	...
	for bb := fn.FirstBasicBlock(); !bb.IsNil(); bb = llvm.NextBasicBlock(bb) {
		...
		current := b.CreateLoad(ptr, chain, "")
		prev := b.CreateLoad(ptr, frame.Prev, "")
		linked := b.CreateICmp(llvm.IntEQ, current, frame.Frame, "")
		b.CreateStore(b.CreateSelect(linked, prev, current, ""), chain)
	}
}
