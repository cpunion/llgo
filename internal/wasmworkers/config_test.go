...
func TestValidateTarget(t *testing.T) {
	if err := (Config{Count: 2}).ValidateTarget("js", "wasm", "emscripten"); err != nil {
		t.Fatal(err)
	}
	if err := (Config{Count: 2}).ValidateTarget("js", "wasm", "emscripten-memory64"); err != nil {
		t.Fatal(err)
	}
	for _, target := range [][3]string{
		{"js", "wasm", ""},
		{"wasip1", "wasm", "wasi-preview1"},
		{"linux", "amd64", ""},
	} {
		if err := (Config{Count: 2}).ValidateTarget(target[0], target[1], target[2]); err == nil {
			t.Fatalf("ValidateTarget(%q, %q, %q) succeeded", target[0], target[1], target[2])
		}
	}
