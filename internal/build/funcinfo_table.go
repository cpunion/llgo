...
func emitFuncInfoEntrySites(ctx *context, pkg llssa.Package) {
	...
	emitFuncInfoEntrySitesForModule(
		pkg.Module(), ctx.prog.PointerSize(), runtimeSiteObjectFormat(ctx),
		runtimeEntrySiteSectionInfo(ctx),
	)
}

func emitFuncInfoEntrySitesForModule(mod llvm.Module, pointerSize int, siteFormat siteObjectFormat, entrySiteInfo siteSectionInfo) {
	if mod.IsNil() {
		return
	}
