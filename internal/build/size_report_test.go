...
func TestParseReadelfOutput(t *testing.T) {
	...
	report := buildSizeReport("fake.bin", parsed)
	if report == nil {
...
func TestParseReadelfRealBinary(t *testing.T) {
	...
	if err := cmd.Start(); err != nil {
		t.Fatalf("start llvm-readelf: %v", err)
	}
	parsed, err := parseReadelfOutput(stdout)
	stdout.Close()
	if err != nil {
		cmd.Process.Kill()
		cmd.Wait()
		t.Fatalf("parse real binary: %v", err)
	}
	if err := cmd.Wait(); err != nil {
		t.Fatalf("llvm-readelf: %v", err)
	}
	report := buildSizeReport(path, parsed)
	t.Logf("sections=%d modules=%d", len(parsed.sections), len(report.Modules))
	if len(report.Modules) == 0 {
		t.Fatalf("no modules for %s", path)
	}
}
