...
func TestConfigureWasmWorkers(t *testing.T) {
	...
	export := crosscompile.Export{WasmABI: crosscompile.WasmABIEmscripten}
...
func TestConfigureWasmWorkersRejectsUnsupportedTargets(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "WASI", conf: Config{Goos: "wasip1", Goarch: "wasm"}, export: crosscompile.Export{WasmABI: crosscompile.WasmABIWASIPreview1}},
		{name: "native", conf: Config{Goos: "linux", Goarch: "amd64"}},
	} {
		t.Run(test.name, func(t *testing.T) {
