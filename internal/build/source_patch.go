...
func applySourcePatchForPkg(base, current map[string][]byte, runtimeDir, goroot, pkgPath string, ctx sourcePatchBuildContext) (bool, map[string][]byte, []string, error) {
	...
	for _, entry := range entries {
		...
		if isPlan9Assembly {
			...
			if _, err := os.Stat(original); err != nil {
				return false, nil, nil, fmt.Errorf("source patch assembly replacement %s has no GOROOT target: %w", filename, err)
			}
	...
	if len(patchSrcs) == 0 && len(assemblyReplacements) == 0 {
		return false, current, nil, nil
	}
	...
	if skipAll {
		...
		for _, entry := range srcEntries {
			...
			if len(skips) != 0 && sourcePatchMayContainSkip(src, skips) {
			...
			if err != nil {
				return false, nil, nil, fmt.Errorf("annotate stdlib source file %s: %w", filename, err)
			}
	...
	for name := range annotations {
		if annotationCounts[name] == 0 {
			return false, nil, nil, fmt.Errorf("source patch annotation target %q was not found in package %s", name, pkgPath)
		}
	}
