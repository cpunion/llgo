...
func TestWasmPreAsyncifyArgs(t *testing.T) {
	...
	if got, want := wasmPreAsyncifyArgs(target, "in.wasm", "out.wasm", true, optlevel.Oz),
		[]string{"-Oz", "--inline-max-combined-binary-size=16384", "-g", "in.wasm", "-o", "out.wasm"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("wasmPreAsyncifyArgs(debug) = %v, want %v", got, want)
	}
	if got := wasmPreAsyncifyArgs(target, "in", "out", true, optlevel.O0); got != nil {
	...
	if got := wasmPreAsyncifyArgs(nil, "in", "out", true, optlevel.O2); got != nil {
