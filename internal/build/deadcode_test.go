...
func TestApplyDeadcodeDropOverridesWritesStrongTypeOverride(t *testing.T) {
	...
	// This unit test exercises only method-table override emission. Construct a
	// minimal destination module directly instead of forging an executable main
	// module, whose coroutine ABI correctly requires a validated startup table.
	entryPkg := &aPackage{
		Package: &packages.Package{PkgPath: "entry_main"},
		LPkg:    ctx.prog.NewPackage("entry_main", "entry_main"),
	}

	if err := applyDeadcodeDropOverrides(nil, []Package{srcAPkg}, entryPkg, false, false); err != nil {
		t.Fatal(err)
	}
...
func TestDCEEntryRootCandidates(t *testing.T) {
	...
	got, err := dceEntryRootCandidates(nil, nil, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	got, err = dceEntryRootCandidates(nil, nil, true)
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(got, want) {
...
func TestDCEEntryRootCandidatesIncludesCExports(t *testing.T) {
	...
	got, err := dceEntryRootCandidates(nil, pkgs, false)
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(got, want) {
