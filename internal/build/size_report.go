...
func reportBinarySize(path, format string) error {
	report, err := collectBinarySize(path)
	if err != nil {
		return err
	}
	switch format {
...
func collectBinarySize(path string) (*sizeReport, error) {
...
func buildSizeReport(path string, data *readelfData) *sizeReport {
	...
	for idx, sec := range data.sections {
		...
		for i := 0; i < len(syms); i++ {
			...
			report.add(moduleNameFromSymbol(sym.Name), sec.Kind, next-addr)
			cursor = next
		}
		if cursor < end {
