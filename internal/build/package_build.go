...
func (ctx *context) canUseIsolatedBackend() bool {
	// A complete coroutine universe freezes exact SSA and LLVM Program
	// identities. In particular, worker/host-operation lowering rejects a
	// package-local Program even when its target is structurally identical.
	// Keep frontend emission in the coordinator Program; the staged coroutine
	// backend releases that Program before independently materializing objects.
	if ctx.clCompilation != nil && ctx.coroEmission != nil &&
		ctx.coroEmission.CompleteRuntimeABI() {
		return false
	}
