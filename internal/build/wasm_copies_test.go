		...
		t.Run(fmt.Sprint(bits), func(t *testing.T) {
			...
			for _, tc := range []struct {
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					if got := lowerWasmAggregateCopies("wasm", td, mod, true); got != want {
					...
					if got := lowerWasmAggregateCopies("wasm", td, mod, true); got != 0 {
			...
			before := mod.String()
			if lowerWasmAggregateCopies("amd64", td, mod, true) != 0 || mod.String() != before {
	...
	for _, bits := range []int{32, 64} {
		for _, roots := range []bool{false, true} {
			t.Run(fmt.Sprintf("%d/roots=%t", bits, roots), func(t *testing.T) {
				...
				if got := lowerWasmAggregateCopies("wasm", td, mod, roots); got != 1 {
				...
				if allocations != 1 || lowerWasmAggregateCopies("wasm", td, mod, roots) != 0 {
