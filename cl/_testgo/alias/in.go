...
// CHECK-LABEL: define ptr @"{{.*}}alias.(*Point).Move$coro"(ptr %0, ptr %1, ptr %2, double %3, double %4){{.*}} {
// CHECK: call void @__llgo_coro_fault_prepare_v1
// CHECK: getelementptr inbounds %"{{.*}}alias.Point", ptr %2, i32 0, i32 0
// CHECK: fadd double
// CHECK: store double
// CHECK: getelementptr inbounds %"{{.*}}alias.Point", ptr %2, i32 0, i32 1
// CHECK: fadd double
// CHECK: store double
...
// CHECK-LABEL: define ptr @"{{.*}}alias.(*Point).Scale$coro"(ptr %0, ptr %1, ptr %2, double %3){{.*}} {
// CHECK: call void @__llgo_coro_fault_prepare_v1
// CHECK: getelementptr inbounds %"{{.*}}alias.Point", ptr %2, i32 0, i32 0
// CHECK: fmul double
// CHECK: store double
...
// CHECK-LABEL: define ptr @"{{.*}}alias.main$coro"(ptr %0, ptr %1){{.*}} {
func main() {
	// CHECK: call ptr @"{{.*}}AllocZ"(i64 16)
	// CHECK: call ptr @"{{.*}}alias.(*Point).Scale$coro"
	// CHECK: call void @__llgo_coro_await_prepare_v3
	// CHECK: call ptr @"{{.*}}alias.(*Point).Move$coro"
	// CHECK: call void @__llgo_coro_await_prepare_v3
