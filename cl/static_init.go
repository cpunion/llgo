...
func (n *staticInitNode) addStore(store staticInitStore) bool {
	leaf := &staticInitNode{
		value: store.value, function: store.function, functionValue: store.functionValue,
...
func (n *staticInitNode) addLeaf(path []staticInitPathElem, leaf *staticInitNode) bool {
	if len(path) == 0 {
		...
		n.value, n.function, n.functionValue = leaf.value, leaf.function, leaf.functionValue
...
func (p *context) buildStaticInitExprNamed(typ types.Type, node *staticInitNode, name string) (llssa.Expr, bool) {
	...
	if node.function != nil {
		...
		// A Dispatch value must be produced by the canonical coroutine
		// function-value lowering. Folding it here would preserve the two-word
		// carrier but incorrectly put a raw body address in its descriptor word.
		// Static initialization is only an optimization, so keep the original
		// package-init instructions and let compileValue materialize the frozen
		// {descriptor, environment} representation.
		if p.coroFunctionValueRequiresDynamicDescriptor(node.functionValue) {
			return llssa.Expr{}, false
		}
