	...
	for len(queue) != 0 {
		...
		if len(block.Instrs) != 0 && len(successors) == 2 {
			if branch, ok := block.Instrs[len(block.Instrs)-1].(*ssa.If); ok {
				if condition, exact := coroPhysicalConstantBool(branch.Cond); exact {
					if condition {
...
// coroPhysicalConstantBool recognizes only values whose result is already a
// complete Go constant at the SSA boundary. x/tools preserves comparisons
// generated by switch lowering as BinOp instructions even when both operands
// are constants; LLVM later folds them, but ProgramIR must select the same CFG
// before effect propagation or a dead call can manufacture a coroutine edge.
// Keep this deliberately narrower than a general SSA evaluator: arithmetic,
// phis, loads, and conversions remain dynamic unless x/tools has reduced them
// to a Const. The type checker guarantees that the admitted comparison is
// valid for its two constant operands.
func coroPhysicalConstantBool(value ssa.Value) (bool, bool) {
	if exact, ok := value.(*ssa.Const); ok {
		if exact.Value == nil || exact.Value.Kind() != constant.Bool {
			return false, false
		}
		return constant.BoolVal(exact.Value), true
	}
	...
	left, leftExact := comparison.X.(*ssa.Const)
	right, rightExact := comparison.Y.(*ssa.Const)
	if !leftExact || !rightExact || left.Value == nil || right.Value == nil ||
		left.Value.Kind() == constant.Unknown || right.Value.Kind() == constant.Unknown {
		return false, false
	}
