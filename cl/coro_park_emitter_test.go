...
func TestCoroParkOperationStatusPlanRejectsIncompleteOrAmbiguousVocabulary(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "duplicate normal", normal: []uint64{1, 1}, abort: 2, shutdown: 3, want: "repeats"},
		{name: "abort collision", normal: []uint64{1}, abort: 1, shutdown: 3, want: "also normal"},
		{name: "shutdown collision", normal: []uint64{1}, abort: 2, shutdown: 2, want: "not distinct"},
		...
		t.Run(test.name, func(t *testing.T) {
			err := validateCoroParkOperationStatuses(test.normal, test.abort, test.shutdown)
			if err == nil || !strings.Contains(err.Error(), test.want) {
				t.Fatalf("status plan error = %v, want %q", err, test.want)
			}
		})
	}
	if err := validateCoroParkOperationStatuses([]uint64{1, 2, 3}, 4, 5); err != nil {
