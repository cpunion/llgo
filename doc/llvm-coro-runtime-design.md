...
- 下一依赖顺序为：实现 v1 plain Dispatch descriptor 的 producer/consumer，使无捕获、NoSuspend、单 plain body 的动态 Go callback 保持源码同步调用风格且不复制函数主体；再用跨包高阶 summary/字段流证明 TLS `parameter → field → load → call` 的 closed target，未知或混合目标继续 fail closed。随后补齐 `go` spawn、park/wake，扩展 CFG/递归 coroutine lowering，并插入和验证 loop/recursion/long-block 抢占 poll。CoroRoot init/main 必须等通用 CFG/synthetic-init child-await lowering完成后开放。不得把 catalog 当作启动列表，也不得用扩大线性 allowlist或 runtime function-pointer fallback 绕过这些生命周期协议。
