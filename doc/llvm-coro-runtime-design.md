...
- 多 P 优先使用owner-assisted有界批量sharing；只有profiling证明必要时才引入并发
  lock-free deque。缓存frame等优化不得改变continuation所有权协议。
...
- `//export`统一ingress adapter的版本化ABI、物理入口和panic/Goexit退出能力；
...
Native已经把该core接到完整物理路径：初始8项inline、其余64项稳定分页的10,000 logical-slot M目录、每route active-M slot、
...
隔离和restart/stop线性化。未解锁G退出又以clean successor覆盖program、peer和temporary
replacement owner；command main-return遇不可取消blocked M时按Go进程语义终止。per-call
create/join现已升级为有界8项standby-M缓存：wrapper只在Go入口返回后发布Returned，parent
凭exact physical token、scalar slot和仍为Returned的baton确认后才回收logical slot并开放复用；
缓存满时strong join，shutdown先join standby再停止factory。独立physical-thread ledger从
...
当前Native多P使用更轻的owner-assisted模型：

- 每个P拥有无分配的intrusive FIFO，本地enqueue/dequeue均为O(1)，精确
  `readyCount`用于批量决策；queue link永远不由其他P并发读取。
- 空闲P只向fleet发布无指针、可合并的需求。源P在一个stable physical action后认领需求，
  把约一半、最多一个固定mailbox容量的P-neutral runnable批量转移，并至少保留一个本地
  runnable；目的P只需一次request/doorbell。
- 多个源P可以竞争同一需求，但CAS只允许一个winner；目的mailbox的Try gate允许普通竞争
  立即退回，不让scheduler owner自旋。
- 外部线程、timer、poller和host callback仍只向exact source发布POD fact并请求其owner；
  source-affine cleanup完成、`ResumePacket`物化后，G才进入上述sharing。
- 不引入concurrent victim deque、随机读取其他P的queue或第二套global runnable owner。
  如果profiling证明owner-assisted路径不足，未来lock-free deque只能替换本地queue/ingress
  实现，不能改变demand、P-neutral proof、mailbox root和Imported门。
- 每执行固定数量本地任务仍检查source/request debt，防止外部事件和全局需求饥饿。
- Preempted G 放队尾。
...
### 16.4 单 executor tinygogc（baremetal/WASM）

- `AllocRoot`、G task storage和LLVM coroutine frame都使用普通tinygogc allocation，显式free为no-op；scheduler先exact unlink/clear，后续GC回收物理block。
- Scheduler全局P/G、ready/timer/wait链必须始终到达每个live G或其frame内部wait record；G再到达完整frame chain。collector扫描当前executor stack和静态global后递归保守扫描heap object，因此会扫描整块LLVM frame allocation，包括CoroSplit写入的pointer spill。它不需要全局“当前root chain”，也不需要从函数地址反查或提取post-CoroSplit offset。
- 这个方案要求non-moving、stop-the-world和serialized executor。LLVM frame地址从`coro.begin`到destroy固定；collector运行期间没有另一executor并发写frame，所以LLVM自动spill store不需要并发write barrier。
- `wasip1-gc`是当前显式opt-in的WASM纵向档：target的`gc: conservative`由编译器转成私有`llgo_wasm_gc`能力，用户/标准库不能伪造。`gc: precise`和WASI threads组合均fail closed。
- 真实验收让child只在suspended timer frame中持有两级heap对象；另一个G先通过test-only direct C leaf以volatile非指针值覆盖16 KiB native resume stack，再连续执行forced GC、8192个同尺寸保活分配和第二次GC，child恢复后逐字段校验。fresh LLGo build与Wasmtime执行均不依赖BDWGC、libuv、Asyncify或每G机器栈。
- 目前只证明单executor frame-aware conservative retention、普通不可达对象回收以及可编译的`memory.grow`适配；没有对增长路径做独立压力验收，也没有证明多executor STW、precise/moving GC、timer weak lease、Finalizer/Cleanup/weak/unique或全部GOROOT GC语义。对应能力仍为`Degraded`/`Unavailable`，不能标成Go GC Full。
- Baremetal GC只能在scheduler或已注册StopSafepoint运行；plain allocator可用`collectWithInitiatorStack`同步收集，ISR不能触发分配/GC。当前tinygogc mutex为空，所有target都不得把该档用于并行executor。
...
- LLGo 已固定使用 `cpunion/llvm` 的 LLVM 22-only 绑定。该分支吸收上游 LLVM 22 的完整 switch API 变更，并保留 LLGo 所需的 switched-resume builder/CoroSplit API；编译标签、工具链配置和当前 CI 均不再维护 LLVM 21 及以下版本。
...
- 多基本块 CFG、聚合值、PHI 和抢占 lowering 已完成。自然循环、循环入口及每 64 条有效指令的长直线块插入 poll；scheduler 的 P 级原子 request 只有在 slow path 才执行 publish/yield/`llvm.coro.suspend`，fast path 不切换。LLVM 22 有 native64/wasm32 pre-/post-CoroSplit 与 object 测试。
...
- compiler runtime现独占`poll.c`及scheduler-facing符号，轻量runtime不再依赖标准库
  ...
  `test/*`、Go 1.26 GOROOT、LLVM 22和各target CI仍是合并前验收，不得由这些
  focused结果外推为完整标准库兼容。
...
- Channel source已经去掉native profile的1024个同时live operation硬上限。target-neutral
  ...
  goroutine链已编译运行通过。通用operation ready index以每page两个32-bit leaf word和
  511-page summary跳过空前缀，不分配per-event node且只依赖32-bit CAS；producer按
  `mailbox -> leaf -> summary -> pending`发布，owner先清summary再读leaf，因此cursor后到达
  的事实仍sticky到下一epoch。slot 1025的32-reduction gate防止executor退回线性scan；同一
  万goroutine逆序唤醒实测由约53.1秒降到两次fresh运行的1.865–2.501秒（对应Go为
  0.539–0.545秒）。
- Phase 36之后的主要缺口仍是工程闭环而非新的coroutine可行性障碍：零/单source
  ...
  event/host/HAL adapter；JS command timer和WASI Preview 1 timer/file reactor已经完成。native退出实验模式仍必须满足35.1与35.2，尤其仓库
...
- `wasip1`、`wasip2` 和 `wasm-unknown` 默认仍明确选择 leaking/nogc frame backend，不依赖 libuv 或 BDWGC；另有显式`wasip1-gc`选择单executor tinygogc conservative档。后者已用真实`runtime.GC`在child timer frame suspended期间执行两轮collection和8192个同尺寸压力分配，随后恢复并验证两级pointer graph；产物由Wasmtime运行且无`GC_*`符号。它没有改变默认target，也不允许WASI threads/precise GC。`wasip2` 与 `wasm-unknown` 已通过真实 `llgo build -target=...`、wasm magic/symbol closure、无 `GC_*`/undefined 检查，并由 wasmtime 运行返回 0。普通`wasip1` executable现在固定为单线程、自有并导出memory的command profile；`-nostartfiles`保证LLGo `_start`拥有startup，并在managed initial slice返回后进入固定64-operation C reactor。该reactor不分配、不建立第二个scheduler，以一个`poll_oneoff`集合等待absolute alarm和fd read/write，已运行并发timer及`syscall.Open/Read/Write/Seek/Close/Unlink`。Preview 1网络明确`ENOSYS`，`path_open`/unlink/metadata的同步host import只具备Degraded compensation，不能保证慢host filesystem期间其他G前进；高层`os.File`还受通用interface exact-effect缺口阻挡。当前`wasip2`产物仍只是Preview 2目标的core module，不是WIT component，也没有对应pollable component lifecycle。
- frame allocator已有conservative BDWGC、nogc/WASM malloc、tinygogc/baremetal和opt-in tinygogc/WASM后端。WASM conservative档通过static P/G→frame chain扫描整块CoroSplit allocation，不增加root-chain或offset ABI。精确frame root map、多executor STW/barrier、weak timer/finalizer与cleanup语义尚未实现，不能据此宣称完整Go GC兼容。
- deterministic scheduler core 已能管理多个 frame、ready queue、park/wake、稳定 wait registration/cancel、绑定 P 的 target-neutral executor driver、正常 main-return cancellation、terminal panic frame destruction和 idle/requested/stopping/disabled 状态。Native production runner使用固定8-route topology；route 1收养command M，其余route各由一个joinable pthread M拥有，所有M复用同一reducer和route-local source/reactor。target-neutral原子execution quota只包围真正的managed resume，缺少permit的route仍可服务timer/poll/channel/cancel/transfer；缩容不撤销in-flight resume，扩容通过sticky waiter与doorbell关闭lost-wake窗口。locked-M blocking call会detach active resume并把同一P/driver/source执行域交给临时replacement M；replacement沿公共reducer继续处理同route事件，原M在C返回后确认exact current lineage并release到有界standby cache、重获quota并精确restore。未解锁退出的M由clean successor永久替换，command main-return不会等待不可取消的blocked M。core race、真实1→4→1物理并发、单配额同/跨route补偿、channel/timer/socket poll、嵌套replacement/retirement E2E、standby/`SetMaxThreads`上限对抗以及普通标准库`runtime.GOMAXPROCS`与`runtime/debug.SetMaxThreads`同步调用已经通过。跨route channel/select进程重复启动仍有约5%的既有generic reducer invalid-stop竞态，已由未修改基线1,000次对照确认，不属于standby/线程账本回归，但必须作为下一项core gate关闭。尚未接入registration unregister枚举、dynamic/sharded timer heap、完整callback/reentry teardown、完整panic/defer/recover/Goexit和全量标准库/GOROOT矩阵。
- 2026-07-27更新：上述约5%的跨route channel/select invalid-stop及随后捕获的低概率
  `coroRunSlice` Invalid均已定位并关闭。第一处发生在strong command shutdown允许channel peer
  先完成物理commit、随后把continuation逻辑决议改为Canceled时；cleanup cursor现分别记录
  physical status与logical delivery，只在Canceled决议下允许丢弃已commit结果，同时继续拒绝
  普通completed loser。第二处位于同一source-neutral cleanup progression：channel `ApplyOne`
  已关闭source但仍需等待一个admitted producer退出时返回`RetryBudget`，合法的Bound cleanup +
  parkDetaching + waitSetWorkResolving此前在Promote被误判Invalid。Bound阶段现复用普通wait-set
  分类，RetryBudget/dirty重新排队、AwaitExternal离队等待，只有parkReady才消费park并进入typed
  materialization。确定性held-admission回归、core race、host/wasm adapter和fresh linked跨route
  高压共同覆盖这两个不变量。
...
- 可选的profile-driven lock-free/local deque；不得复制demand/mailbox所有权状态机。
- Frame pooling和内存上限。
- Precise frame maps。
...
| `runtime/debug` | Stack/GC/SetMaxThreads等 | `SetMaxThreads`已接入统一物理M账本；Stack/GC仍依赖logical frames与STW | `SetMaxThreads` native已完成，其余工具阶段 |
...
request/ring、确认exact current lineage已Returned并release到standby、重获permit，再精确restore。单配额同route channel、timer、
socket readiness和nested blocking linked E2E均已通过。raw replacement pthread由有界8项
