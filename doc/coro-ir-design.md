...
状态：2026-07-23架构硬切换收口。hidden runtime helper、intrinsic/call-elision、physical proof/implicit-fault、physical emission session/ordinary-compiler isolation、single-event Park protocol、channel/WaitSet Park envelope、ordinary semantic recipe/local Effect-Exec、await/spawn physical control choice、channel/select physical operation choice、panic/outcome/cleanup choice以及remaining call/value/worker choice的封闭cohort已经切换；Phase R的唯一native target、唯一logical Park、统一source dispatcher和mandatory stackless架构也已完成。其他LoweringFacts仍主要是report/cache identity，whole-function emitter仍在迁移，因此本状态表示“旧runtime并行轨道已物理删除”，不表示完整语言、GC或平台能力已经完成
...
- 2026-07-27更新：上一条的5.8%/5.3% gate及随后捕获的低概率`coroRunSlice` Invalid均已关闭。
  ...
  committed-result discard。普通loser仍须pending，故没有放宽select唯一winner约束。第二处
  根因是source-neutral cleanup promotion只接受`AwaitingExternal`，而一个已关闭但仍有admitted
  producer的channel `ApplyOne`会返回`RetryBudget`，形成合法的Bound + Detaching + Resolving
  状态却被误判Invalid。Bound cleanup现复用普通wait-set收敛：RetryBudget/dirty重新排队、
  AwaitExternal离队等待，只有parkReady才消费park并进入typed materialization。确定性测试持有
  producer admission覆盖该精确状态，core race、host/wasm adapter及fresh linked跨route压力均
  已通过。
...
- 旧`currentCoro`production访问为零；协程专用lowerer的body capability访问已降到39处并由精确AST gate限定，whole-function emitter尚未完成，因此这里仍是compiler下一阶段的主要债务。
- `CoroPlan/EmissionUniverse`直接权威读取的精确预算为364，已经由各replacement cohort持续下降，但距离窄的function/site plan消费边界仍有差距。
- production `EnableCoro*`阶段开关为零；`None`和coroutine profile选择已经删除，stackless LLVM coroutine是唯一架构。target能力只由冻结的`TargetCapabilities`和catalog派生。
...
| channel send/recv | typed hchan与route-aware operation已有direct vertical slice | channel operation以64槽page按需单调增长并由两级durable ready index跳过空page/slot；Go 1.26 GOROOT的1万goroutine链已通过；仍需typed payload precise-GC和完整语言/race矩阵 |
...
ProgramIR CFG上entry到terminal的最长路径，并在每个精确call occurrence加入callee的transitive cost/certificate；
所有target必须已发布leaf/DAG或imported outcome entry，call count必须与ProgramIR一致，超预算、大result native slot、CFG/call cycle及任何开放边
...
plain call或完整panic/recover已经闭环。当前outcome DAG已具有content-addressed path certificate、physical
recipe重建gate以及CoroSplit前后和最终LLVM优化后的结构验证；LLVM报告的仍是abstract IR work而非target cycle。
完整抢占保证仍需要把同一模型扩展到所有plain/runtime no-cut region，并补最终machine/runtime cost certificate。
...
- fixed small source capacity适合allocation-constrained profile；target-neutral wait/timer/poll/worker/channel/keyed-wait 的默认 page 是64槽。Channel现可由owner在bind期间按需单调发布稳定page，保持既有两字OperationID，最大32,704槽；native按实际park并发增长，embedded/baremetal仍可保留显式静态容量。Channel已复用通用64槽leaf + 511页summary的两级ready index，万goroutine逆序唤醒从约53.1秒降到两次fresh运行的1.865–2.501秒；对应Go基线为0.539–0.545秒。native Timer当前独立配置64页即4096槽，其他尚未动态化的common source配置16页即1024槽，worker使用4个物理pthread与1024-job ring，task-control仍为8槽。timer/poll后续可复用同一ready index或deadline heap；容量策略不改变compiler IR。
...
spawn begin/commit
await prepare/inline select
await inline finish
await destroy/consume or resumed consume
...
- depth bound只允许留下规范的`pendingAwait`并fail-open到原有scheduler路径，不允许产生第二份或半提交transaction；
...
runtime core只返回`Declined/Started/Suspend/Destroy`状态并维护frame/completion ownership；`llvm.coro.resume/done/destroy`只存在于编译器生成的caller，使LLVM 22能够看见完整静态handle lifetime并执行frame elision。因而WASM、RTOS和baremetal可以复用同一状态机和同一LLVM lowering，只需提供各自的executor/event入口，不引入pthread、libuv或BDWGC依赖。
...
架构不要求每G native stack、libuv、BDWGC或pthread，但这只是兼容候选，不是平台完成度。当前production target adapter中，llgo native Linux/Darwin已有opt-in有界fleet：route 1原位收养program executor，启动时创建固定7个pthread M；所有domain共用唯一物理run-step reducer，各自通过exact idle gate和route-local poll set等待doorbell/fd/deadline。Program target负责全部peer与共享worker pool的start/stop/join及route/backend/driver强关闭。Worker保持单物理池并用每job的`OperationID.Route`支持fleet completion，不做函数地址反查；pthread入口只携带scalar M slot，C11 ring支持多个owner并发reservation。P-neutral result物化、demand injection/work sharing、运行期逻辑`runtime.GOMAXPROCS`、locked-M同P replacement、owner succession、standby M缓存和标准`runtime/debug.SetMaxThreads`已经接通；仍未提供批量/local-deque steal、完整affinity或callback/reentry shutdown闭环。缺少filesystem、process、socket或host async能力的平台仍按target capability决定可用package。LLVM支持范围只是19–22，不考虑19以下版本。
...
4. **唯一stackless架构**：production不存在coroutine enable/profile选择；PhysicalABI、ChildAwait、Bootstrap、Channel、Worker、NativeFleet等阶段开关不再组成配置笛卡尔积。LLVM stackless lowering始终启用，target capability由冻结的`TargetCapabilities`/catalog派生，测试特例留在test-only builder。
...
14. outcome-plain已启用path-sensitive direct-call DAG certificate和post-LLVM结构gate；完整whole-program
    MaxAtomicCost仍需覆盖所有plain/runtime no-cut region，并补target-machine/runtime cost certificate。
...
生成owner-scoped snapshot；build在任何package codegen前把该snapshot装入当前`CoroPlanDigest v33`、
...
1. 先提交当前双owner fleet、单一Park/source和mandatory stackless硬切换及六项fresh E2E结果，不再混入新能力。
