...
2. **生产freeze/build/SSA/digest接线（已落地）**：EmissionUniverse为managed-required集合中的
   ...
   `PlanDigestSchema` 当前为 `llgo.coro.plan-digest.v31`。同一physical `(symbol, ABI)`可以对应多个
...
- total callable identity、generic callable certificate、exact TrustedInline invocation以及
  managed physical entry/atomic-cost proof进入当前 `PlanDigestSchema v31`，事实变化会改变digest；
...
- library-effect-summary v3的exact foreign producer→archive→consumer fixed point、显式本地
...
- EmissionUniverse freeze、`internal/build`分类、SSAFunctionPolicy/SSAPlan与
  当前 `PlanDigestSchema v31` 的端到端传递；
