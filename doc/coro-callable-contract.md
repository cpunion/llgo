...
这仍不是完整Go affinity语义。当前locked foreign call已在同一P上创建temporary replacement；
locked G若未unlock就终止，也会通过target-neutral retire receipt和clean-factory succession
物理退休/替换program、peer或replacement M。真实TLS destructor E2E已证明旧pthread退出；
main-return遇不可取消locked C时按Go进程语义直接终止，不会无限join。WASM、WASI、
embedded和baremetal目前只有目标中立的logical domain模型，没有等价的production affinity
backend。后续完成门仍包括普通locked park/preempt、standby M缓存、`SetMaxThreads`、更完整的
defer/panic/Goexit/cancel、callback/reentry和非command shutdown矩阵。
...
- 当前每route reactor、固定8-route fleet、动态逻辑execution quota、demand injection、locked-M同P replacement及owner succession不能外推为scalable shard、批量/local-deque stealing、standby M、全部callback/reentry/非command shutdown或完整affinity已经完成。
...
2. **生产freeze/build/SSA/digest接线（已落地）**：EmissionUniverse为managed-required集合中的
   ...
   `PlanDigestSchema` 当前为 `llgo.coro.plan-digest.v33`。同一physical `(symbol, ABI)`可以对应多个
...
10. **并行、平台验收与清理（部分完成）**：native route-aware submission、固定8-route M/P target、
    ...
    parked-result packet、fleet demand injection/work sharing、locked-M同P replacement、owner succession和TCP E2E已落地；仍需AwaitCapacity、
    queued cancel、standby M、`SetMaxThreads`、callback/reentry与非command shutdown矩阵、批量/local-deque steal和完整affinity。2026-07-23已用真实generated-cgo/Python序列确认共享worker
...
- total callable identity、generic callable certificate、exact TrustedInline invocation以及
  managed physical entry/leaf或DAG atomic-cost certificate进入当前 `PlanDigestSchema v33`，事实变化会改变digest；
...
- library-effect-summary v5的exact foreign producer→archive→consumer fixed point、显式本地
  冲突拒绝、identity-only保守路径、same-M physical lowering、managed entry/cost proof与
  transitive re-publication。
...
- EmissionUniverse freeze、`internal/build`分类、SSAFunctionPolicy/SSAPlan与
  当前 `PlanDigestSchema v33` 的端到端传递；
- declaration的progress/affinity/reentry/memory保守投影、wrapper全body分析，以及与
  ...
  在调用场景中授权，managed plan仍保留`WaitForeign`；
...
- 现有scheduler/operation核心的OperationID、generation、ParkState、WaitSetRecord、
  ...
  materialization、fleet demand injection/work sharing、locked-M同P replacement、未解锁owner succession和TCP E2E。这些证明callable模型有可复用
  底座，不证明standby M、`SetMaxThreads`、callback/reentry shutdown、批量/local-deque steal、通用backend recipe或各平台已完成。
...
- 第一版`runtime.LockOSThread`、locked G exact P/M归属、同M foreign call、generation-bound replacement lineage、未unlock退出的clean-M succession及main-return blocked-M终止已经闭环；仍缺
  standby M缓存、`SetMaxThreads`联动和完整callback/reentry shutdown矩阵。共享worker继续
