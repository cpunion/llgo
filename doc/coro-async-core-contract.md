...
这已经证明固定fleet内对P-neutral initial、yielded和已物化park continuation的通用按需注入/工作共享，以及双M/双P启动、运行、停止和join；它仍不是动态`GOMAXPROCS`、无界global run queue、批量/local-deque随机steal或完整affinity策略。当前native实际活动domain仍固定为2，后续扩容只能复用这一个需求/transfer协议，不能再增加spawn专用旁路。
...
| --- | --- | --- |
| Native Linux/Darwin | POSIX monotonic clock + 每route pipe doorbell/`poll` reactor + 共享bounded pthread worker | opt-in fleet profile已接入program target：command M/route 1与peer pthread M/route 2并行运行独立P/source catalog，program start/stop、peer join、共享4-worker/1024-job MPMC ring、Timer/Poll/Worker/Channel exact route和强关闭均已闭环；双owner TCP stdlib probe及10,000次压力通过。固定fleet已用无指针demand和exact mailbox通用转移P-neutral initial/yield/materialized-park runnable，并以Imported门防止首次运行前回弹；dynamic GOMAXPROCS、批量/local-deque steal、完整affinity、GC/语言/GOROOT矩阵仍未完成，因此不是完整Go runtime兼容 |
...
2. `ExecutorFleet`的程序接入、双M owner/stop/join、worker lifecycle、route-local reactor、Timer/Poll/Worker exact route，以及零/单source、Channel/select、HostOp deadline和keyed/private-registry park的typed `ResumePacket`物化已完成；固定fleet的通用P-neutral demand injection/work sharing也已完成。下一并行核心是动态P数量、批量/local-deque steal、affinity和blocking compensation；每个尚未证明P-neutral的park继续禁止迁移。
...
5. 零/单Timer、Manual、Poll、Worker以及Channel/select、Worker+Timer deadline和runtime-private registry/transport已经通过固定`ResumeCleanupKind + POD plan`在进入可迁移ready状态前物化到compiler/runtime提供的P-neutral `ResumePacket`，并结束原route result/cleanup lease。固定fleet的demand injection只接受该P-neutral证明；新P不得回访旧source取得payload或执行cleanup，未覆盖的新source/park默认继续禁止迁移。
