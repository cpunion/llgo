...
每次locked blocking call现在先尝试复用有界8项standby-M缓存；缓存为空才由clean factory创建pthread。temporary lineage返回时，parent仍持有exact Returned baton，C wrapper以generation-bound physical token加scalar M slot确认Go入口已经返回；只有该确认完成后logical M目录slot才回收，raw pthread才可被下一slot重新分配。缓存满时立即strong join，target close按peer、worker、standby、factory顺序强关闭。10,000仍只是logical slot/retired tombstone/并发深度的fail-closed上限，不是预创建线程数。独立物理线程账本从默认10,000开始，精确覆盖program、clean factory、4 worker、7 peer、successor/replacement及standby pthread；创建前reserve，join或永久pthread exit后release，`runtime/debug.SetMaxThreads`按Go的MaxInt32 clamp、返回旧值和低于当前M数即fatal语义直接操作该账本。C并发/restart/8项溢出、14线程上限下连续replacement复用、program-successor terminal close以及普通标准库`runtime/debug.SetMaxThreads`同步调用均已通过。Command root由G尾部padding中的一字节`AnyOwner/CurrentOwner` mobility保持在program P，首次入队建立隐式owner，唯一跨P mailbox只接受AnyOwner，因此没有每G P指针或affinity registry；peer→program反向spawn验证main return仍由正确边界发布。owner-assisted bounded half-batch现已替代逐G demand transfer；它仍不是无界global run queue、并发local-deque随机steal或完整thread/realm affinity策略。local deque仅是profiling驱动优化；完整标准库/GOROOT的LockOSThread、syscall、callback/reentry及非command host shutdown矩阵仍需验收。后续扩展继续复用同一个需求/transfer与handoff协议，不能增加spawn、source或native-only所有权旁路。
