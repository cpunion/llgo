...
当前native fleet profile已经把这条core接入真实程序：route 1原位收养command线程上的program P，启动时按正整数`GOMAXPROCS`或online CPU数量选择1–8个连续route，其余route各由一个joinable pthread M拥有；每个domain有独立P/driver/source catalog、pipe doorbell和POSIX poll set，并执行同一个物理reducer。活动前缀在pthread启动前冻结，关闭时统一stop、逐M join、逐route strong-close，未选择suffix保持零状态。C叶只有一个静态C→Go owner边界，pthread参数只是`uint32 route`，不传函数地址、G、P或coroutine handle。固定8槽`RunnableTransferMailbox`接受never-run root、普通`SuspendYield`以及已进入`parkMaterialized`的`SuspendPark` continuation；slot以generation和FIFO exact import持有唯一GC root。G复用既有对齐padding中的一字节`transferState`，Published期间关闭pointer-only preempt gate并拒绝普通`Enqueue`，因此不会被第三个P重复取得，且native/wasm32的G大小不变。
...
这已经证明1–8 P有界fleet内对P-neutral initial、yielded和已物化park continuation的通用按需注入/工作共享。1/4/8 route core生命周期通过race测试，`GOMAXPROCS=1`的单P channel park/progress与`GOMAXPROCS=8`的物理peer启动、分发、停止和全join也通过真实linked E2E。Command root由G尾部padding中的一字节`AnyOwner/CurrentOwner` mobility保持在program P，首次入队建立隐式owner，唯一跨P mailbox只接受AnyOwner，因此没有每G P指针或affinity registry；4 P peer→program反向spawn验证main return仍由正确边界发布。它仍不是运行期`runtime.GOMAXPROCS(n)`调整、无界global run queue、批量/local-deque随机steal或完整thread/realm affinity策略；后续扩展继续复用同一个需求/transfer协议，不能增加spawn专用旁路。
...
- allocation-free `ExecutorFleet`核心、route registry和P-neutral runnable transfer已通过core/race测试并接入native程序入口；route 1由command线程执行，启动期选择的其余route由独立pthread M并行执行，所有domain共用同一物理reducer并各自拥有P/source catalog/doorbell/reactor。普通空domain通过同一`IdleArmed -> final scan -> CommitSleep`事务进入standby，由routed request精确唤醒；1–8 P启动、shutdown publication、route strong-join、全部peer join和driver/source回收已形成完整target lifecycle。所有已支持Park source在旧owner物化为P-neutral packet后才允许迁移；
...
| --- | --- | --- |
| Native Linux/Darwin | POSIX monotonic clock + 每route pipe doorbell/`poll` reactor + 共享bounded pthread worker | opt-in fleet profile已接入program target：启动按`GOMAXPROCS`环境或online CPU选择1–8个M/P，program start/stop、全部peer join、共享4-worker/1024-job MPMC ring、Timer/Poll/Worker/Channel exact route和强关闭均已闭环；双owner TCP stdlib probe及10,000次压力通过。无指针demand和exact mailbox可通用转移P-neutral initial/yield/materialized-park runnable，并以Imported门防止首次运行前回弹；运行期`GOMAXPROCS` resize、批量/local-deque steal、完整affinity、GC/语言/GOROOT矩阵仍未完成，因此不是完整Go runtime兼容 |
...
Native表中的有界fleet已从production-island推进为可执行target profile；它证明无栈continuation可在启动期选择的1–8个真实M/P间并行，并能用统一event/worker模型运行标准库TCP链。它仍不把启动期active prefix外推为运行期P resize、全部可运行G迁移或完整标准库兼容。
...
2. `ExecutorFleet`的程序接入、启动期1–8 M/P owner/stop/join、worker lifecycle、route-local reactor、Timer/Poll/Worker exact route，以及零/单source、Channel/select、HostOp deadline和keyed/private-registry park的typed `ResumePacket`物化已完成；通用P-neutral demand injection/work sharing也已完成。下一并行核心是运行期P resize、批量/local-deque steal、affinity和blocking compensation；每个尚未证明P-neutral的park继续禁止迁移。
