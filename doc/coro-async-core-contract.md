...
3. 以已完成的host-owned pull core、host-target V2 entry、JS/WASM command timer pump和显式Node file/network runner为边界，把同一operation协议接入JS command HostOp/HostReentry，并把action/continuation协议接入WASI `poll_oneoff`/pollable reactor、RTOS HAL和baremetal IRQ/alarm/WFI loop；受控file/network runner证据不升格为尚未接线平台的内建启动/runtime E2E。
