...
- `OperationID`已冻结为两字`source:8 + route:9 + local:15 + generation:32`；route在runtime instance内单调分配且永不复用，关闭后留下永久tombstone，Manual/TaskControl/Poll/Worker producer可只凭POD ID投递精确executor，Timer V2的record/lease也使用相同exact route且由owner时钟驱动。当前`ExecutorFleet`、program executor原位收养、Timer/Poll/Worker route、固定8-route物理M owner/stop/join、动态逻辑execution quota、共享worker lifecycle、route-local reactor、locked-M replacement、三类owner succession、有界standby M与统一`SetMaxThreads`账本均已接入native target。零/单Timer、Manual、Poll、Worker以及typed Channel/select、单Worker HostOp、Worker+Timer deadline和keyed/private-registry park的`parkReady`现由旧owner物化到P-neutral `ResumePacket`后才可迁移；需求claim、exact mailbox和Imported门开放了通用P-neutral global injection/work sharing。owner-only intrusive queue现在用精确计数一次迁移约一半、最多8个continuation，并只请求一次target；其他P不读取victim link，因而不新增并发deque状态机。仍未完成的是普通locked park/preempt、完整affinity以及callback/reentry与非command shutdown矩阵，不能由有界profile外推为完整native multi-P。
...
2. `ExecutorFleet`的程序接入、固定8-route M/P owner/stop/join、动态逻辑execution quota、标准`runtime.GOMAXPROCS`、worker lifecycle、route-local reactor、Timer/Poll/Worker exact route，以及零/单source、Channel/select、HostOp deadline和keyed/private-registry park的typed `ResumePacket`物化已完成；通用P-neutral demand injection/work sharing、owner-assisted bounded half-batch、locked-M同P replacement、三类owner succession、command main-return blocked-M终止、有界standby M缓存和标准`runtime/debug.SetMaxThreads`也已完成。下一并行核心是普通locked park/preempt、完整affinity以及callback/reentry与非command shutdown矩阵；每个尚未证明P-neutral的park继续禁止迁移。并发local deque只保留为profiling驱动优化，不作为正确性gate。
