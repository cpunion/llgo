...
2. `ExecutorFleet`的程序接入、双M owner/stop/join、worker lifecycle、route-local reactor、Timer/Poll/Worker exact route，以及零/单source、Channel/select和HostOp deadline的typed `ResumePacket`物化已完成。下一并行核心是完成keyed/private-registry cleanup，再开放通用global injection/steal、动态P数量和affinity；每个未完成物化的park继续禁止迁移。
