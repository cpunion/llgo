...
这已经证明有界8-route fleet内对P-neutral initial、yielded和已物化park continuation的通用按需注入/工作共享。1/4/8 route core生命周期通过race测试，固定topology下`GOMAXPROCS=1`的channel park/progress、`GOMAXPROCS=8`的物理peer启动/分发/停止/全join以及运行期1→4→1并发上限变化均通过真实linked E2E；普通同步Go源码也已直接调用标准`runtime.GOMAXPROCS`与`runtime.SetDefaultGOMAXPROCS`入口。Command root由G尾部padding中的一字节`AnyOwner/CurrentOwner` mobility保持在program P，首次入队建立隐式owner，唯一跨P mailbox只接受AnyOwner，因此没有每G P指针或affinity registry；peer→program反向spawn验证main return仍由正确边界发布。它仍不是无界global run queue、批量/local-deque随机steal、完整thread/realm affinity或blocking compensation策略；后续扩展继续复用同一个需求/transfer协议，不能增加spawn专用旁路。
