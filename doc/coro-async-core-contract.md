...
| --- | --- | --- |
| Native Linux/Darwin | POSIX monotonic clock + pipe doorbell + `poll` reactor + bounded pthread worker | compiler command entry已是single-P V2固定循环，已有channel/spawn/panic、Timer V2与Poll V2 production-island证据；`ExecutorFleet`核心和两个独立domain的native production-island已通过，Timer/Poll exact route及Poll callback ingress也已闭环，但尚未接入program entry和双domain reactor arm循环。真实worker submission还未使用route-aware fleet，已park G也不能steal；因此还不是完整native multi-P或标准库E2E |
...
- `OperationID`已冻结为两字`source:8 + route:9 + local:15 + generation:32`；route在runtime instance内单调分配且永不复用，关闭后留下永久tombstone，Manual/TaskControl/Poll producer可只凭POD ID投递精确executor，Timer V2的record/lease也使用相同exact route且由owner时钟驱动。当前`ExecutorFleet`核心、Timer/Poll route和native双domain production-island已完成，但程序entry仍是single-P V2 driver，fleet尚未接入为program multi-P entry；worker submission、双domain reactor arm循环、`parkReady`的P-neutral ResumePacket、global injection和parked work stealing仍未完成，不能由island外推为完整native multi-P。
...
2. 在已通过的`ExecutorFleet`核心、native双domain island及Timer/Poll exact route上，完成程序entry切换、route-aware worker submission、双domain reactor arm循环和parked-result物化；在此之前不开放parked work stealing。
