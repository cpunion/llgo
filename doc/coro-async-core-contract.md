...
- `OperationID`已冻结为两字`source:8 + route:9 + local:15 + generation:32`；route在runtime instance内单调分配且永不复用，关闭后留下永久tombstone，Manual/TaskControl/Poll/Worker producer可只凭POD ID投递精确executor，Timer V2的record/lease也使用相同exact route且由owner时钟驱动。当前`ExecutorFleet`核心、Timer/Poll/Worker route、native双domain production-island和route-local固定reactor pass已完成，但程序entry仍是single-P V2 driver，fleet尚未接入为program multi-P entry；worker pool lifecycle、物理M owner/stop/join循环、`parkReady`的P-neutral ResumePacket、global injection和parked work stealing仍未完成，不能由island外推为完整native multi-P。
