...
当前typed plan仍未接受HostOp deadline和带runtime-private cleanup的keyed/host adapter。双source deadline必须同时退休Worker与Timer；runtime-private registry/transport也必须先完成对应固定枚举cleanup。它们复用同一个plan与runner边界，不能以callback/interface或迁移后回访旧P来绕过。
