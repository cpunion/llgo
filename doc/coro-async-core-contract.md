...
- allocation-free `ExecutorFleet`核心、route registry和P-neutral runnable transfer已通过core/race测试；native两个独立domain的production-island已跑通，同local/generation completion能按`OperationID.Route`精确进入各自mailbox和request gate，两个domain也已通过与程序single-P相同的物理reducer执行并迁移P-neutral yield任务；普通空domain现在可通过同一`IdleArmed -> final scan -> CommitSleep`事务进入standby，并由routed request精确唤醒，command main的“无parked G即完成”语义没有因此放宽；route-local固定poll set又已跑通真实pipe readiness、Poll V2 route ingress、doorbell、新owner epoch、typed result消费和最终G销毁的host/race纵向测试；
...
这些是core-first证据，不是标准库兼容性证明。native compiler command entry已使用single-P V2固定循环；双domain `ExecutorFleet`目前仍未替换程序入口，但已能把现有program P/driver/source/registry作为route 1原位收养，并以fleet自有固定存储创建route 2，不复制或重绑几百处program全局状态。Timer catalog、Poll V2 callback与Worker transport现已纳入exact route：Timer由各domain owner的单调时钟扫描，不暴露producer callback；Poll和Worker的两字`OperationID`可在route admission内完成source publication、exact executor request、duplicate/stale分类和关闭强汇合，Worker仍复用一个进程级固定物理池。该结果尚未把物理owner M、native reactor wait循环与worker pool start/stop接到program-level target，也没有完成已park G的result物化/迁移与work stealing。target-neutral host-owned pull adapter已完成core/race与JS/WASM、WASIp1、baremetal、显式embedded交叉编译验证：`more`只发布later-turn action，idle只暴露POD executor/generation/epoch/deadline，alarm/notification取消后才可复用epoch，shutdown seal两个ingress并strong-join callback tail。compiler的host-target V2 entry也已完成：它只执行一次有界initial slice，保留host callback/reference roots，仅接受canonical `Complete`或executor slot/generation/epoch/flags/deadline均精确的`Yielded`/`Suspended` tuple，后两者detached返回host，不在entry内继续调度或递归re-entry。这仍只是embedding reactor ABI：普通JS/WASM或WASI `_start`没有外部reactor消费`next_action`、调用callback/continuation，因此不能称platform E2E。JS microtask/timeout pump、WASI reactor `poll_oneoff`、RTOS HAL与baremetal IRQ/WFI glue、完整cleanup/defer/recover/Goexit lowering以及precise/moving GC仍是明确缺口。

最新的执行器收口又前进了一步：program single-P与fleet domain现已共用同一个物理run-step reducer，fleet外层只保留exact domain owner、显式单调时钟和budget，不再存在第二套resume/destroy/commit语义。空fleet P也已使用公共timer-aware idle事务完成standby admission；提交后先释放owner epoch，route-local固定poll set执行一次fault-containment有界物理pass，doorbell/fd/deadline唤醒后再取得新epoch并执行公共`WakeExecutorAt`全source重扫。普通domain最后一个G销毁也已与command终止分开：只完成该G并保持executor可接收未来routed work。现有program executor的原位收养、外部registry精确请求以及强关闭后引用清理也已通过core/race；分布式command停止、实际物理M owner循环和worker pool lifecycle接入仍是程序multi-P前的硬边界。

Worker transport的route-aware核心现已完成：仍只有一个进程级固定4线程/1024-job物理池，多个exact P owner仅在无锁reservation word上短暂竞争，胜者在compiler-enforced no-suspend hook内submit或cancel。Job不增加指针或全局operation目录，直接使用既有两字`OperationID.SourceSlot`中的route；完成端按compiler-reserved target profile静态选择legacy program或fleet ingress，fleet再完成exact source/request/doorbell投递。C11环已通过route 1/2交错wrap和四consumer测试，两个互斥profile的真实LLGo plan均验证为唯一raw-only、no-suspend、direct-plain C-worker回调入口。尚未完成的是让program-level fleet coordinator实际start/stop该池，因此当前标准库运行仍使用single-P profile。

#### 少量标准库能力探针政策

核心契约完成后，只保留少量、固定且使用普通Go源码的标准库程序作为能力探针。最小纵向门使用`time.Sleep`验证timer/park/deadline，使用标准库固定导出的`syscall.Open/Write/Seek/Read/Close/Unlink`封装完成一字节文件回环，验证bounded worker/syscall/result ownership；完整`time.Timer`验证controlled Timer V2上的Go timer基本语义，高层`os.File`验证interface/defer与regular-file worker封装，loopback TCP验证readiness/deadline/close-cancel链路。这五项现均已在native single-P下compile-link-run通过。首个失败点必须归纳为缺失的通用compiler、runtime core、source或target adapter契约并在该层修复；不得添加按库名、函数名或fixture匹配的特例opcode/lowering，也不得绕过统一executor/event模型。

`LLGO_CORO_STDLIB_ACCEPTANCE`的`time`、`syscall-file`、`file`、`tcp`和完整`timer`程序已冻结上述源码约束，且 host Go 参考运行可以通过。五项当前均已取得LLGo fresh compile-link-run全绿证据；其中`tcp`在2026-07-20的fresh gate中完成完整编译、native链接与loopback运行，`go test`墙钟约208秒。它们只证明各自冻结的垂直用例，不等于完整`time`、`os`、`syscall`、`net`或GOROOT兼容。
...
| `tcp` | loopback `ListenTCP/DialTCP/AcceptTCP`，顶层无捕获`go serve()`；一字节I/O，20ms read deadline超时，清除后读取60ms延迟回复 | 真实Go 1.26 stdlib源码已完成effect/emission plan、compiler lowering、LLVM验证、native link和loopback运行；覆盖Poll V2 readiness、deadline超时、清除deadline后读取延迟回复及顶层static spawn。它不证明Poll fleet route、并发close/大量连接、完整`net`/resolver矩阵或WASM/WASI backend；WASM当前仍只有lowering/CoroSplit/module compile证据 |
...
net.TCPListener/TCPConn
  ...
  -> Park
  -> single-P POSIX poll set: doorbell + active fd directions + earliest timer/deadline
...
`sync.Once`、`sync.Cond`、FD mutex 和 close barrier 所需的 semaphore/notifyList 也必须 park logical G，不能退回在唯一 executor thread 上等 pthread cond。当前 keyed-wait namespace 已分开 semaphore 与 notify ticket，冻结的native single-P TCP标准库探针已经通过上述链路；完整FD并发、close竞态、multi-P迁移和GOROOT矩阵仍待整体验收。

#### 平台边界

| 平台 | 模型映射 | 2026-07-20 production 现状 |
| --- | --- | --- |
| Native Linux/Darwin | POSIX monotonic clock + pipe doorbell + `poll` reactor + bounded pthread worker | compiler command entry已是single-P V2固定循环，已有channel/spawn/panic、Timer V2与Poll V2 production-island证据；`ExecutorFleet`核心和两个独立domain的native production-island已通过，Timer/Poll/Worker exact route及回调ingress也已闭环，但尚未接入program entry、worker pool lifecycle和双domain reactor arm循环。已park G仍不能steal；因此还不是完整native multi-P或标准库E2E |
...
Native表中的双domain production-island现已用production reducer执行并迁移P-neutral yield任务，且route 1可原位收养已绑定program executor；因为它还没有并行M owner和完整停止/reactor/worker target协调，平台完成度结论不变。
...
- `OperationID`已冻结为两字`source:8 + route:9 + local:15 + generation:32`；route在runtime instance内单调分配且永不复用，关闭后留下永久tombstone，Manual/TaskControl/Poll/Worker producer可只凭POD ID投递精确executor，Timer V2的record/lease也使用相同exact route且由owner时钟驱动。当前`ExecutorFleet`核心、program executor原位收养、Timer/Poll/Worker route、native双domain production-island和route-local固定reactor pass已完成，但程序entry仍由single-P V2 driver独占物理执行；worker pool lifecycle、物理M owner/stop/join循环、`parkReady`的P-neutral ResumePacket、global injection和parked work stealing仍未完成，不能由island外推为完整native multi-P。
...
2026-07-20的执行顺序是“核心能力优先，少量标准库程序只作定位探针”：

1. `StringCat`、泛型`llgo.index`、Darwin sysctl bridge、visibility-only `go:linkname`、`*ssa.MakeMap`及promoted managed wrapper唯一发射阻塞已经越过；真实`time`、`timer`、`syscall-file`、`file`与`tcp`探针均已compile-link-run通过。下一步优先把这五项固化为CI门、完成全编译器/runtime回归并转向multi-P与平台reactor，同时保持“只修generic core、不写package/function白名单”的门槛；不得由这些探针通过外推全部标准库兼容。
2. 在已通过的`ExecutorFleet`核心、native双domain island及Timer/Poll/Worker exact route上，完成程序entry切换、worker pool lifecycle、双domain reactor arm循环和parked-result物化；在此之前不开放parked work stealing。
...
1. netpoll/readiness source；Poll V2 current-owner、typed recipe及native single-P TCP E2E已通过，fleet route、其他平台backend和完整netpoll矩阵仍待完成。
2. completion/worker ForeignOp source；已有bounded worker/core、scalar result、route-aware fleet transport及`syscall-file`/高层`file`真实E2E证据；program-level fleet lifecycle、完整syscall族和取消/容量矩阵仍待完成。
