...
- `OperationID`已冻结为两字`source:8 + route:9 + local:15 + generation:32`；route在runtime instance内单调分配且永不复用，关闭后留下永久tombstone，Manual/TaskControl/Poll/Worker producer可只凭POD ID投递精确executor，Timer V2的record/lease也使用相同exact route且由owner时钟驱动。当前`ExecutorFleet`、program executor原位收养、Timer/Poll/Worker route、双物理M owner/stop/join、共享worker lifecycle和route-local reactor均已接入native target。零/单Timer、Manual、Poll、Worker的`parkReady`现由旧owner物化到P-neutral `ResumePacket`后才可迁移；仍未完成的是Channel/select/HostOp的typed multi-source plan、通用global injection/steal、dynamic P policy和affinity，不能由固定双domain profile外推为完整native multi-P。
...
2. `ExecutorFleet`的程序接入、双M owner/stop/join、worker lifecycle、route-local reactor、Timer/Poll/Worker exact route及零/单source `ResumePacket`已完成。下一并行核心是用固定typed plan完成Channel/select、deadline和runtime-private cleanup的多source物化，然后开放通用global injection/steal、动态P数量和affinity；每个未完成物化的park继续禁止迁移。
