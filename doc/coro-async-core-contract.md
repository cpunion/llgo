...
| --- | --- | --- |
| Native Linux/Darwin | POSIX monotonic clock + pipe doorbell + `poll` reactor + bounded pthread worker | compiler command entry已是single-P V2固定循环，已有channel/spawn/panic、Timer V2与Poll V2 production-island证据；`ExecutorFleet`核心和两个独立domain的native production-island已通过，但尚未接入该program entry。真实worker submission还未使用route-aware fleet，Timer/Poll尚被fleet bind明确拒绝，已park G也不能steal；因此还不是完整native multi-P或标准库E2E |
...
- `OperationID`已冻结为两字`source:8 + route:9 + local:15 + generation:32`；route在runtime instance内单调分配且永不复用，关闭后留下永久tombstone，Manual/TaskControl producer可只凭POD ID投递精确executor，Timer V2的record/lease也使用相同exact route。当前`ExecutorFleet`核心和native双domain production-island已完成，但程序entry仍是single-P V2 driver，fleet尚未接入为program multi-P entry；worker submission、Timer/Poll fleet route、`parkReady`的P-neutral ResumePacket、global injection和parked work stealing仍未完成，不能由island外推为完整native multi-P。
...
2. 在已通过的`ExecutorFleet`核心和native双domain island上，完成程序entry切换、route-aware worker submission、Timer/Poll route和parked-result物化；在此之前不开放parked work stealing。
...
2. completion/worker ForeignOp source；已有bounded worker/core、scalar result及`syscall-file`/高层`file`真实E2E证据；route-aware multi-P production submission、完整syscall族和取消/容量矩阵仍待完成。
