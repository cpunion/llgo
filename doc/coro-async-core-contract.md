...
当前实现仍按每次locked blocking call创建并join一个pthread，尚未增加可复用standby-M缓存；10,000是slot/retired tombstone/并发深度的fail-closed上限，不是预创建线程数，也还未与`runtime/debug.SetMaxThreads`动态设置联动。Command root由G尾部padding中的一字节`AnyOwner/CurrentOwner` mobility保持在program P，首次入队建立隐式owner，唯一跨P mailbox只接受AnyOwner，因此没有每G P指针或affinity registry；peer→program反向spawn验证main return仍由正确边界发布。它仍不是无界global run queue、批量/local-deque随机steal或完整thread/realm affinity策略；完整标准库/GOROOT的LockOSThread、syscall、callback/reentry及非command host shutdown矩阵也仍需验收。后续扩展继续复用同一个需求/transfer与handoff协议，不能增加spawn、source或native-only所有权旁路。
...
- `OperationID`已冻结为两字`source:8 + route:9 + local:15 + generation:32`；route在runtime instance内单调分配且永不复用，关闭后留下永久tombstone，Manual/TaskControl/Poll/Worker producer可只凭POD ID投递精确executor，Timer V2的record/lease也使用相同exact route且由owner时钟驱动。当前`ExecutorFleet`、program executor原位收养、Timer/Poll/Worker route、固定8-route物理M owner/stop/join、动态逻辑execution quota、共享worker lifecycle、route-local reactor、locked-M replacement及三类owner succession均已接入native target。零/单Timer、Manual、Poll、Worker以及typed Channel/select、单Worker HostOp、Worker+Timer deadline和keyed/private-registry park的`parkReady`现由旧owner物化到P-neutral `ResumePacket`后才可迁移；需求claim、exact mailbox和Imported门开放了通用P-neutral global injection/work sharing。仍未完成的是批量/local-deque steal、普通locked park/preempt、完整affinity、standby M缓存、`SetMaxThreads`以及callback/reentry与非command shutdown矩阵，不能由有界profile外推为完整native multi-P。
...
2. `ExecutorFleet`的程序接入、固定8-route M/P owner/stop/join、动态逻辑execution quota、标准`runtime.GOMAXPROCS`、worker lifecycle、route-local reactor、Timer/Poll/Worker exact route，以及零/单source、Channel/select、HostOp deadline和keyed/private-registry park的typed `ResumePacket`物化已完成；通用P-neutral demand injection/work sharing、locked-M同P replacement、三类owner succession和command main-return blocked-M终止也已完成。下一并行核心是standby M缓存、`SetMaxThreads`、批量/local-deque steal、普通locked park/preempt、完整affinity以及callback/reentry与非command shutdown矩阵；每个尚未证明P-neutral的park继续禁止迁移。
