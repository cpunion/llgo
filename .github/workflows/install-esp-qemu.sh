...
case "$OS" in
  ...
  linux)
    ...
    ;;
  mingw*|msys*|cygwin*)
    # Espressif publishes the Windows emulator as an x86-64 MinGW build.
    # It runs natively on hosted x64 runners and through Windows' x64
    # emulation on ARM64 development machines; this is the emulator host
    # architecture, independent of the ESP firmware target architecture.
