...
standard-library/benchmark jobs:

```sh
gh workflow run gc-validation.yml --repo cpunion/llgo \
  --ref <branch> -f scope=all
```

Manual dispatch requires the workflow to be registered on the repository's
default branch; `workflow_call` is also available for focused development
branches. The regular GOROOT workflow is not replaced or redirected.

Use `-f scope=embedded` to rerun only the ESP jobs after a fixture/platform
fix, without consuming runners for already-validated host or Wasm code.

On relevant pull requests, core and ESP jobs run automatically. The five Wasm
profiles reuse the compiler and runners in the existing full-acceptance
sentinel jobs for `dev/test_gc_wasm.sh`; the original Go pressure workloads
remain in the full GOROOT shards. The standalone Wasm matrix is dispatch-only
so normal CI does not build and execute the same workloads twice.
...
This focused gate does not replace final-head five-profile integration suites,
native/patch coverage or `cprintf`, `println`, `fmtprintf` size benchmarks.
Canceled earlier acceptance runs do not count as completed validation.
