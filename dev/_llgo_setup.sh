...
_llgo_compute_bin_path() {
	...
	if [ -z "$gobin" ]; then
		...
		if [ "$gohostos" = "windows" ]; then
			# A Windows drive colon is part of the path, not GOPATH's list
			# separator. Windows Go uses semicolons for multiple GOPATH roots.
	...
	if [ "$gohostos" = "windows" ] && command -v cygpath >/dev/null 2>&1; then
		# dev wrappers run under Git/MSYS Bash on Windows. Convert either an
		# explicit GOBIN or the GOPATH-derived directory before invoking it.
