...
func TestWriteBenchmarkReport(t *testing.T) {
	...
	for _, want := range []string{
		...
		"| macOS | `BenchmarkRuntimeGetG` | 110.000 ns/op | +10.0% |",
...
func TestWriteBenchmarkReportWithoutMain(t *testing.T) {
	...
	err := writeBenchmarkReport(reportOptions{
		currentData: current,
		mainData:    filepath.Join(dir, "missing.js"),
		output:      output,
		seriesURL:   "series",
		sourceURL:   "source",
		runURL:      "run",
	...
	if !strings.Contains(string(data), "| n/a |") {
		t.Fatalf("report without main has no n/a delta:\n%s", data)
	}
}
...
func TestReportHelpers(t *testing.T) {
	...
	if got := formatDelta(1, nil); got != "n/a" {
