...
func writeBenchmarkReport(options reportOptions) error {
	...
	body.WriteString("| Platform | Benchmark | Current | vs main |\n")
	body.WriteString("|---|---|---:|---:|\n")
	for _, platform := range []string{"Linux", "macOS"} {
		...
		for _, name := range expectedGoBenchmarks {
			...
			fmt.Fprintf(&body, "| %s | `%s` | %s ns/op | %s |\n",
				platform, name, formatNumber(value), formatDelta(value, optionalGoBenchmarkValue(mainSuite, name)))
		}
	}
	body.WriteString("\n</details>\n\n")
