...
func (b Builder) MapUpdate(m, k, v Expr) {
	...
	ret := b.Call(b.Pkg.rtFunc(helpers.Assign), typ, m, arg)
...
// MapRuntimeHelperNames is the single lowering decision shared by the LLSSA
// map emitter and compiler preflight. KeyNeedsTemporary records the generic
// pointer-key ABI, whose mapKeyPtr recipe allocates and stores one temporary
// key before the runtime call. Fast scalar, pointer, channel, and string keys
// pass their value directly and therefore do not emit that allocation.
type MapRuntimeHelperNames struct {
	Access1           string
	Access2           string
	Assign            string
	Delete            string
	KeyNeedsTemporary bool
}

// MapRuntimeHelpers returns the exact runtime-helper family selected for the
// effective map type and this program's pointer width. Keeping the decision in
// LLSSA prevents analysis and physical emission from independently guessing
// whether a later map lowering uses the generic or fast ABI.
func (p Program) MapRuntimeHelpers(mapType types.Type) (MapRuntimeHelperNames, bool) {
...
func mapRuntimeHelperNames(kind mapFastKind) MapRuntimeHelperNames {
	return MapRuntimeHelperNames{
		Access1:           kind.accessName(false),
		Access2:           kind.accessName(true),
		Assign:            kind.assignName(),
		Delete:            kind.deleteName(),
		KeyNeedsTemporary: kind == mapFastNone,
	}
}
