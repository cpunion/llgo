...
// IsNativeFuncBackground reports whether background uses a one-word native
// function pointer rather than LLGo's managed Go function-value ABI.
func IsNativeFuncBackground(background Background) bool {
	return background == InC || background == InStdcall
}

func isNativeFuncBackground(background Background) bool {
	return IsNativeFuncBackground(background)
}
