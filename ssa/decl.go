...
func (p Package) newFunc(
	...
	if v, ok := p.fns[name]; ok {
		...
		if v.NeedsEnv() {
			p.Prog.markClosureEnvFunction(v.impl, 0)
		} else if sig, ok := v.raw.Type.(*types.Signature); ok {
...
func (p Function) Param(i int) Expr {
	...
	return p.PhysicalParam(physical)
}
...
func (p Function) PhysicalParam(i int) Expr {
	physical := i
	physicalCount := len(p.params)
	if p.env != nil {
		physicalCount++
	}
	if i < 0 || i >= physicalCount {
		panic(fmt.Sprintf("ssa: physical parameter index %d out of range [0, %d)", i, physicalCount))
	}
	if p.env != nil {
		if i == 0 {
