...
func TestSetBlockExAtStartAcceptsEmptyPhysicalBlock(t *testing.T) {
	prog := NewProgram(nil)
	defer prog.Dispose()
	pkg := prog.NewPackage("empty", "example.com/empty")
	fn := pkg.NewFunc("empty.start", NoArgsNoRet, InGo)
	body := fn.MakeBody(1)
	prefix := fn.NewBuilder()
	prefix.SetBlockEx(fn.Block(0), AtStart, false)
	prefix.AllocaT(prog.Uintptr())
	prefix.Dispose()
	body.Return()
	body.EndBuild()
	body.Dispose()
	if err := llvm.VerifyModule(pkg.Module(), llvm.ReturnStatusAction); err != nil {
		t.Fatalf("verify empty-block start insertion: %v\n%s", err, pkg.String())
	}
}
