...
func TestProgramMapRuntimeHelpersOwnsGenericAndFastSelection(t *testing.T) {
	prog := NewProgram(nil)
	defer prog.Dispose()

	fast, exact := prog.MapRuntimeHelpers(types.NewMap(types.Typ[types.Int], types.Typ[types.Int]))
	if !exact || fast.KeyNeedsTemporary || fast.Access1 == "MapAccess1" || fast.Assign == "MapAssign" {
		t.Fatalf("map[int]int helpers = %+v, %t; want direct fast-key family", fast, exact)
	}
	genericKey := types.NewStruct(
		[]*types.Var{types.NewField(token.NoPos, nil, "value", types.Typ[types.Uint64], false)},
		nil,
	)
	generic, exact := prog.MapRuntimeHelpers(types.NewMap(genericKey, types.Typ[types.Int]))
	if !exact || !generic.KeyNeedsTemporary || generic != (MapRuntimeHelperNames{
		Access1: "MapAccess1", Access2: "MapAccess2", Assign: "MapAssign", Delete: "MapDelete", KeyNeedsTemporary: true,
	}) {
